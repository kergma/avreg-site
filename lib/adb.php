 ...
 */
   public function add_user($u_host, $u_name, $passwd, $groups, $guest, $pda, $u_devacl, $u_layouts, $u_forced_saving_limit, $sessions_per_user,$limit_fps,$nonmotion_fps, $limit_kbps, $session_time, $session_volume, $u_longname, $remote_addr, $login_user) {
   	$query = sprintf('INSERT INTO USERS 
         ...
         MAX_VIDEO_FPS, MAX_VIDEO_NONMOTION_FPS, MAX_MEDIA_SESSIONS_RATE_KB,
         MAX_MEDIA_SESSIONS_MINUTES, MAX_MEDIA_SESSION_VOLUME_MB,
 ...
 */
   public function get_users($status = false) {
      $users = array();
      $query = 'SELECT ALLOW_FROM, USER_LOGIN, GUEST, PDA, PASSWD, STATUS, ALLOW_CAMS, ALLOW_LAYOUTS, MAX_FORCED_REC_MINUTES,  MAX_MEDIA_SESSIONS_NB, MAX_VIDEO_FPS, MAX_VIDEO_NONMOTION_FPS, MAX_MEDIA_SESSIONS_RATE_KB, MAX_MEDIA_SESSIONS_MINUTES, MAX_MEDIA_SESSION_VOLUME_MB,LONGNAME, CHANGE_HOST, CHANGE_USER, CHANGE_TIME '.
         'FROM USERS ';
      if ($status)
      ...
      while ($res->fetchInto($line, DB_FETCHMODE_ASSOC)) {
         $users[] =array(
            ...
            'MAX_MEDIA_SESSIONS_RATE_KB' => trim($line[$this->_key('MAX_MEDIA_SESSIONS_RATE_KB')]),
            'MAX_MEDIA_SESSIONS_MINUTES' => trim($line[$this->_key('MAX_MEDIA_SESSIONS_MINUTES')]),
