...
foreach ($result as $row  )
{
   ...
   $ui['MAX_FORCED_REC_MINUTES'] = is_null($row['MAX_FORCED_REC_MINUTES'])?NULL:(int)$row['MAX_FORCED_REC_MINUTES'];
   ...
   $ui['MAX_MEDIA_SESSIONS_RATE_KB'] = is_null($row['MAX_MEDIA_SESSIONS_RATE_KB'])?NULL:(int)$row['MAX_MEDIA_SESSIONS_RATE_KB'];
   $ui['MAX_MEDIA_SESSIONS_MINUTES'] = is_null($row['MAX_MEDIA_SESSIONS_MINUTES'])?NULL:(int)$row['MAX_MEDIA_SESSIONS_MINUTES'];
   $ui['MAX_MEDIA_SESSION_VOLUME_MB'] = is_null($row['MAX_MEDIA_SESSION_VOLUME_MB'])?NULL:(int)$row['MAX_MEDIA_SESSION_VOLUME_MB'];
...
$reverse=FALSE //менеяет местами ключи со значениями в рез-й разметке
	...
	foreach ($assoc_array as $k => $v)	{
		...
		$a .= '<div><label for="id_'.$_name.$key.'" class="class_'.$_name.' ">'.$text_prefix.$value.'</label>'."\n";
		$a.='</div>';
		$a.='</div>';
	}
