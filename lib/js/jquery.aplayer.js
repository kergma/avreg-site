...
//		$('img', this).removeAttr('height').removeAttr('width').css({'height':'', 'width':'' });
    ...
    $.fn.aplayerIsFlowPlayer = function(){
  		if($(this).find('.flowplayer.'+$.aplayer.classElMedia).size()>0) return true;
  		else return false;
    };    
...
//			$.aplayer.config.Show();
		...
		versionPredicate : function(f,s){
			...
			showImage : function(container, settings){
				...
				{
					//  title="'+settings.type+'"
					$('<img  src="'+settings.src+'" class="'+$.aplayer.classElMedia+'" name="img"/>').appendTo(container);
					return;
				}
				
				var size = 'style="width:'+settings.width+'px; height:'+settings.height+'px; "';
				// title="'+settings.type+'"
				var im = $('<img class="'+$.aplayer.classElMedia+'" src="'+settings.src+'" '+size+' name="img"/>').attr({'height':settings.height, 'width':settings.width });
...
//				.append('<param name="play" value="true" >');
           ...
           createObj_MP3:function(settings){
            ...
            loadFlowPlayer : function(){
            	if(!$.aplayer.config.isFlowPlayerLoaded && !$.aplayer.config.isFPLoadingProgress){
            		...
            		$.getScript($.aplayer.config.path_prefix+'flowplayer/aplayerfncs.js' , function(data){
            			...
            			$.getScript($.aplayer.config.path_prefix+'flowplayer/flowplayer-3.2.11.min.js' , function(data){
        	            	//тестовая задержка
        	            	//alert('load');
            ...
            setFlowPlayer : function(container, settings){
            	...
            	var Autostart = (settings.autostart == "on" || settings.autostart == "true" || settings.autostart == true)? true:false;
                var Controls =(settings.controls=='embed' || typeof(settings.controls)=='undefined' )? {}:null;
            	...
            	var player = $f(id,  
            		{
	            	    src: $.aplayer.config.path_prefix+"flowplayer/flowplayer-3.2.15.swf",
	            	    wmode: "opaque" 
            	    },
            	    {
            	    clip: {
            	    ...
            	    onLoad : function(){
            	    	$('#'+id+' object[id^='+id+']').addClass($.aplayer.classElMedia+' flowplayer');
            	    	if(Controls==null){
            	    		$.aplayer.setControls(container, settings);
            	    	}else{
            	    		//Создаем субконтейнер для медиа-элемента, вставлем в него медиа элемент и помещаем в контейнер плеера
            	    		var con_h = $(container).height();
                			var SubCont = $('<div style="overflow:hidden; position:relative;"></div>').addClass($.aplayer.classMediaCont).height(con_h);
            	    		//переставляем flowplayer в субконтейнер
            	    		$('#'+$.aplayer.idElMedia + settings.nr+', #'+$.aplayer.idContainer+settings.nr+'_api' )
                			.appendTo($(SubCont));
            	    		//Устанавливаем субконтейнер медиа-элемента и панель контролов в медиа плеер HTML5
            	            $('#'+$.aplayer.idContainer + settings.nr ).append($(SubCont));
...
//			var Scale = $($.aplayer.ControlBar.scale).css($.aplayer.ControlBar.ControlsContainers);
			...
			$('img', ControlBar).addClass('cntr_img');
...
*/
            ...
            //Создаем субконтейнер для медиа-элемента, вставлем в него медиа элемент и помещаем в контейнер плеера
			var SubCont = $('<div style="overflow:hidden; position:relative;"></div>').addClass($.aplayer.classMediaCont).css({'overflow':'hidden'}).height(meHeight);
			...
			if($('#'+$.aplayer.idContainer+settings.nr).aplayerIsFlowPlayer()){
				$f($.aplayer.idContainer+settings.nr)
