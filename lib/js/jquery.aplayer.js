...
function checkType(typeElemMedia, fileNameElemMedia){
	...
	this.getExtension = function (){
		var sub = this.fileName.substr(this.fileName.lastIndexOf('.') + 1, this.fileName.length - this.fileName.lastIndexOf('.') + 1).toLowerCase();
		sub += '&asdad=asdasd';
		var RegExp = /[^&]+/;
	...
	this.detectVLCPlugin = function() {
        ...
        if (navigator.plugins && (navigator.plugins.length > 0)) {
            ...
            try {
                ...
                var test = $('<object classid="' + $.aplayer.clsid + '" codebase="http://downloads.videolan.org/pub/videolan/vlc/last/win32/axvlc.cab"></object>');
	 ...
	 * @type {{avi: string, mp4: string, flv: string, ogg: string, ogv: string, webm: string, mp3: string, swf: string, other: string}}
	 */
	this.defaultPlugin = {
		'avi' : 'vlc',
		'mp4' : 'flowplayer',
...
//---------------------------------------------------- Player ------------------------------------
    ...
    $.fn.aplayerIsImage = function(){
  		...
  		if($(el).attr('name')=='img' && $(this).children('div').children('div').attr('t')== undefined) return true;
  		else return false;
    };
    ...
    $.fn.aplayerIsMjpegImage = function(){
        ...
        if($(el).prop('tagName')=='IMG' && $(this).children('div').attr('t')== undefined) return true;
        else return false;
    };
	...
	$.aplayer = {
        ...
        togglePtzAreas: function (aplayerID, show, callback) {
            ...
            if (show) {
                var $dfdAreaRight = $.get('/avreg/online/ptz-right.php');
                var $dfdAreaBottom = $.get('/avreg/online/ptz-bottom.php');

                $.when($dfdAreaBottom, $dfdAreaRight)
                    .done(function (areaRight, areaBottom) {
                    ...
                    .fail(function(){
                        ...
                        window.console.debug('was failed to load ptz areas');
                    })
                    .always(function(){
...
//        clsid:"clsid:9BE31822-FDAD-461B-AD51-BE1D1C159921",
        clsid:"clsid:E23FE9C6-778E-49D4-B537-38FCDE4887D8",
			...
			showVLC : function(container, settings){
                ...
                if($.browser.msie){
                    html+= '<div class="'+ $.aplayer.classMediaCont+'" style="overflow:hidden; position:relative; height:'+settings.height+'px; width:'+settings.width+' ">';
                        html+= '<OBJECT classid="'+$.aplayer.clsid+'" codebase="http://downloads.videolan.org/pub/videolan/vlc/last/win32/axvlc.cab" ';
            		...
            		html+='<embed type="application/x-vlc-plugin" name="vlc" autoplay="'+Autostart+'" loop="yes" \
				        ...
				        target="'+MediaURL+'" src="'+MediaURL+'" volume="100" toolbar="' + toolbarShow + '"></embed>';
...
//						console.log(error.message);
            ...
            getDurationElement : function (Elem){
                ...
                if(MediaElt==undefined) return '00:00:00';
