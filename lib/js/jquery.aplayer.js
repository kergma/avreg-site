/**
 * Объект, который проверяет, можно ли воспроизести fileNameElemMedia через typeElemMedia
 * @param typeElemMedia - Тип медиаэлемента (flowplayer, embed, rtsp, vlc или еще что-то)
 * @param fileNameElemMedia - Имя воспроизводимого файла
 */
function checkType(typeElemMedia, fileNameElemMedia) {
    /**
     * Тип элемента
     * @type {string}
     */
    this.type = typeElemMedia.toLowerCase().trim();
    /**
     * Имя воспроизводимого файла
     * @type {string}
     */
    this.fileName = fileNameElemMedia;

    this.getExtension = function () {
        var sub = this.fileName.substr(
            this.fileName.lastIndexOf('.') + 1,
            this.fileName.length - this.fileName.lastIndexOf('.') + 1
        ).toLowerCase();
        sub += '&asdad=asdasd';
        var RegExp = /[^&]+/;
        return sub.match(RegExp)[0];
    };

    /**
     * Расширение воспроизводимого файла
     * @type {string}
     */
    this.extension = this.getExtension();

    /**
     * Типы файлов, которые может воспроизвести html5
     * @type {{video/ogg: string, video/mp4: string, video/webm: string, audio/mp4: string}}
     */
    this.html5Type = {
    ...
    /**
     * Типы файлов, которые может воспроизвести flowplayer
     * @type {{ogg: boolean, mp4: boolean, swf: boolean, mp3: boolean}}
     */
    this.flowPlayerType = {
        'ogg': true,
        'mp4': true,
    ...
    /**
     * Проверяю, подключен ли в браузере flashplugin (true - подключен, false - отсутствует)
     * @returns {boolean}
     */
    this.flashDetectConstruct = function () {
        var a = !1;
        var b = "";
    ...
    /**
     * Поиск поддержки vlc плагина
     * @returns {boolean} - true - если поддерживается, false - иначе
     */
    this.detectVLCPlugin = function () {
        var name = "VLC";
        if (navigator.plugins && (navigator.plugins.length > 0)) {
            ...
            try {
                var vlcFamilies = ['Twoflower', 'Rincewind'];
                var test = $('<object classid="' + $.aplayer.clsid
                    + '" codebase="http://downloads.videolan.org/pub/videolan/vlc/last/win32/axvlc.cab"></object>');
                for (var i in vlcFamilies) {
                    if (test[0].VersionInfo.indexOf(vlcFamilies[i]) > -1) {
                        return true;
                    }
                }
            } catch (e) {
    ...
    this.enableVLCPlugin = this.detectVLCPlugin();

    /**
     * Хранит в себе данные, подключен ли flash
     * @type {boolean}
     */
    this.flashDetect = this.flashDetectConstruct();
    /**
     * Проверяю, может ли плагин воспроизвести файл
     * @type {{
     * Может ли воспроизвести средствами html5-video
     * canPlayHtml5: Function,
     * Может ли воспроизвести rtsp
     * canPlay_rtsp: Function,
      * Может ли воспроизвести flowplayer
     * canPlay_flowplayer: Function,
     * Может ли воспроизвести vlc
     * canPlay_vlc: Function,
     * Может ли воспроизвести emdeb
     * canPlay_embed: Function}}
     */

    var _self = this;
    this.canPlayPlugIn = {
        canPlay_video: function () {
            if (_self.extension == 'ogv') {
                _self.extension = 'ogg';
            }
            var newElem = document.createElement(_self.type);
            newElem.src = _self.fileName;
            var typeElem = _self.type + '/' + _self.extension;
            // todo - заглушка для canPlayType для старых браузеров
            if (newElem.canPlayType && newElem.canPlayType(typeElem + _self.html5Type[typeElem])) {
                return true;
            }
        ...
        canPlay_rtsp: function () {
            if (_self.type !== 'vlc' && (!_self.enableVLCPlugin() || $.browser.ie)) {
                return false;
            }
        ...
        canPlay_flowplayer: function () {
            if (typeof _self.flowPlayerType[_self.extension] !== 'undefined' &&
                _self.flashDetect) {
                return true;
            }
    ...
    /**
     * Собственно проверка на способность плагина воспроизвести медиа-файл
     * @returns {boolean}
     */
    this.checkCanPlayTypeFromPlugin = function () {
        if ((this.type === 'audio') && (this.type === 'video')) {
            if (this.canPlayPlugIn.canPlayHtml5()) {
                return true;
            }
        } else if (typeof this.canPlayPlugIn['canPlay_' + this.type] !== 'undefined') {

            if (this.canPlayPlugIn['canPlay_' + this.type]()) {
                return true;
            }
        }
    ...
    /**
     * Дефолтные настройки плагинов (в случае, если ни один вариант не подошел)
     * @type {{avi: string, mp4: string, flv: string, ogg: string, ogv: string,
     *          webm: string, mp3: string, swf: string, other: string}}
     */
    this.defaultPlugin = {
        'avi': 'vlc',
        'mp4': 'flowplayer',
...
$.fn.addPlayer = function (settings) {
    if (settings.src == null) {
        ...
        $(this).each(function () {
            ...
            if (!$this.hasClass('pl_wrapper')) {
                $this.addClass('pl_wrapper');
            }

            // костыль необходим для поддержания dom-структуры в модуле view.js
            // todo выпилить костыль, упростить DOM плеера, заменить $element.parents('.pl_wrapper') на $element
            $this.append('<div class="pl_cont"></div>');
            $target = $this.find('.pl_cont');

            $.aplayer.play($target, settings);
        });
    }
    return this;
};
...
$.fn.aplayerSetImgSrc = function (imageSource) {
    ...
    $(pl).children(':first-child').attr('src', imageSource);
...
$.fn.aplayerSetSrcSizes = function () {
    $(this)
        .children('[id ^=' + $.aplayer.idContainer + ']')
        ...
        $(Cont).children('video, audio, object, embed, img, .' + $.aplayer.classMediaCont).each(function () {
            if (!($(this).hasClass($.aplayer.classMediaCont))) {
                ...
                if (fp_obj.length > 0) {
                    ...
                    if (sub_h <= fp_h && sub_h > fp_h + fpY) {
                        fpY = sub_h - fp_h;
                    }
                    //привязка к правой границе субконтейнера
                    if (sub_w <= fp_w && sub_w > fp_w + fpX) {
                        fpX = sub_w - fp_w;
                    }
...
$.fn.aplayerSetSize = function (Sizes) {
    ...
    $(this).children('div[id ^=' + $.aplayer.idContainer + ']').each(function () {
        ...
        }).end().children('.' + $.aplayer.classMediaCont).children('video, audio, img').each(function () {
                //Установка размера суб-элемента и вложенного медиа-элемента НЕ ПРОВЕРЯЛАСЬ!!!
                $(this)
                    .height(Sizes.height - $(this).next('div[id ^=' + $.aplayer.idControlPanel + ']').height() - 7)
                    .width(Sizes.width - 6);
            });
    });
    if ($.browser.msie) {
...
$.fn.aplayerResizeContanerOnlyToParent = function () {
    $(this).children('div[id ^=' + $.aplayer.idContainer + ']').each(function () {
        $(this).height($(this).parents('.pl_wrapper').height() - 2);
        $(this).width($(this).parents('.pl_wrapper').width() - 2);
        var Cont = $(this);
        ...
        $(Cont)
            .find('.logoPlay')
            .removeAttr('style')
            .css({'position': 'relative', 'top': (($(Cont).height() - $(Cont)
            .find('.logoPlay').height()) / 2) + 'px' });
    });
    return this;
};
...
$.fn.aplayerResizeToParent = function () {
        $(this).children('div[id ^=' + $.aplayer.idContainer + ']').each(function () {
            var h_cont = $(this).parents('.pl_wrapper').height();
            var w_cont = $(this).parents('.pl_wrapper').width();
            ...
            $cont.find('object, embed, img').each(function () {
                if ($(this).hasClass('cntr_img')) {
                } else {
                    //чтобы избежать ресайза изображений контролов
                ...
                .end()
                .find('.MediaCont')
                .children('video, audio, img, object, embed')
...
$.fn.aplayerIsImage = function () {
    ...
    if ($(el).attr('name') == 'img' && $(this).children('div').children('div').attr('t') == undefined) {
        return true;
    } else {
        return false;
    }
...
//Является ли медиа-элемент картинкой (для online)?
$.fn.aplayerIsMjpegImage = function () {
    var el = $(this).children('div').children('div').children(':first-child');
    if ($(el).prop('tagName') == 'IMG' && $(this).children('div').attr('t') == undefined) {
        return true;
    } else {
        return false;
    }
...
$.fn.aplayerIsEmbedOrObject = function () {
    if ($(this).find('embed, object').size() > 0) {
        return true;
    } else {
        return false;
    }
...
//Является ли внедренный объект vlc
$.fn.aplayerIsVlc = function () {
    if ($(this).find('[type^="application/x-vlc-plugin"]').size() > 0 ||
        $(this).find('[classid^="' + $.aplayer.clsid + '"]').size() > 0) {
        return true;
    } else {
        return false;
    }
...
$.fn.aplayerIsFlowPlayer = function () {
    if ($(this).find('.flowplayer').size() > 0) {
        return true;
    } else {
        return false;
    }
...
$.aplayer = {
     ...
     * Toggles PTZ areas.
     *
     * @param {Number}  aplayerID
     * @param {Boolean} show
     * @param {Function} callback       callback after async operation, returns true as first argument on success
     */
    togglePtzAreas: function (aplayerID, show, callback) {
            var $player = $('#'+aplayerID),
                $win = $player.parents('.win');

            if (show) {
                var $dfdAreaRight = $.get('./ptz-right.php');
                var $dfdAreaBottom = $.get('./ptz-bottom.php');

                $.when($dfdAreaBottom, $dfdAreaRight)
                    .done(function (areaBottom, areaRight) {
                        $win.find('.ptz_area_right').html(areaRight[0]).show();
                        $win.find('.ptz_area_bottom').html(areaBottom[0]).show();
                        $player.parents('.pl_wrapper').addClass('with_ptz');
                ...
                $win.find('.ptz_area_bottom, .ptz_area_right').hide();
                $player.parents('.pl_wrapper').removeClass('with_ptz');
                    $player.parent().aplayerResizeToParent();
            }
    ...
    draggable: function (Container) {
        ...
        if ($(Container) != null && $(Container).height() != null && $(Container).width() != null) {
...
//                        'mov':'vlc'
    ...
    IsConfigurate: false,
    // Является ли медиаэлемент html5-video
    IsHtml5Video: false,
    //начальная инициализация
    init: function (globalSettings) {

        if ($.aplayer.IsConfigurate && globalSettings == null) {
            return;
        }
        ...
        var totalObj = $.extend({},
            //базовые настройки плеера для всех браузеров
            $.aplayer.baseSettings['*'] == null ? {} : $.aplayer.baseSettings['*'],
            //базовые настройки плеера текущего типа браузера
            $.aplayer.baseSettings == null ? {} : $.aplayer.baseSettings[$.browserInfo.browser],
            //внешние конфигурационные настройки для всех браузеров
            globalSettings == null ? {} : globalSettings['*'],
            globalSettings == null ? {} : globalSettings[$.browserInfo.browser]
        ); //внешние конфигурационные настройки текущего типа браузера

        //Дополняем значения
        var baseObj = $.aplayer.baseSettings[$.browserInfo.browser];
        for (var i in baseObj) {
            if (totalObj[i] == undefined) {
                totalObj[i] = {};
            }
            for (var j in baseObj[i]) {
                if (totalObj[i][j] == undefined) {
                    totalObj[i][j] = {};
                }
                for (var k in baseObj[i][j]) {
                    if (totalObj[i][j][k] == undefined) {
                        totalObj[i][j][k] = baseObj[i][j][k];
                    }
                }
            }
        }

        baseObj = $.aplayer.baseSettings['*'];
        ...
        for (var i in arrVer) {
            ...
            if (typeof(totalObj[arrVer[i]]) == 'undefined' && arrVer[i] == $.browserInfo.version) {
                break;
            }
            if (typeof(totalObj[arrVer[i]]) == 'undefined') {
                continue;
            }
        ...
        $.each(curSets, function (protocol, data) {
            ...
            $.each(data, function (i, value) {
                value = value.trim();
                var typeObj = new Array();
                if (value.search(',')) {
                    typeObj = value.split(',');
                    for (var i in typeObj) {
                        typeObj[i] = typeObj[i].trim();
                        typeObj[i] = typeObj[i].toLowerCase();
                    }
                } else {
                    typeObj[0] = value.toLowerCase();
                    console.log(typeObj[0]);
                }
                for (var i in typeObj) {
                    if (typeObj[i] != 'embed' &&
                        typeObj[i] != 'video' &&
    ...
    versionPredicate: function (f, s) {
        ...
        if (F.length > S.length) {
            for (var i = S.length; i < F.length; i++) {
                S[i] = 0;
            }
            p = 1;
        } else if (F.length < S.length) {
            for (var i = F.length; i < S.length; i++) {
                F[i] = 0;
            }
            p = -1;
        }
        ...
        for (var i = 0; i < F.length; i++) {
            if (parseInt(F[i]) > parseInt(S[i])) {
                return 1;
            } else if (parseInt(F[i]) < parseInt(S[i])) {
                return -1;
            }
        }
    ...
    MIMEtypes: {
        ...
        mjpg: 'multipart/x-mixed-replace',
        tiff: 'image/bmp',
        bmp: 'image/bmp',
        png: 'image/png',
        jpeg: 'image/jpeg',
    ...
    setApplicationType: function (extension, elementMediaType, settings) {
        ...
        if ($.aplayer.MIMEtypes[extension] != null) {
            ...
            elementMediaType = 'application/' + extension;
        }
    ...
    setType: function (settings) {
        ...
        if (settings.type != null) {
            if (settings.type.indexOf('image') != -1) {
                return $.extend(settings, {mediaType: 'image' });
            }
            if (settings.type.indexOf('application') != -1) {
                return $.extend(settings, {mediaType: 'embed' });
            }
        }
        ...
        if (settings.protocol == 'rtsp') {
            ...
            if ($.aplayer.config[settings.protocol][settings.extension] != null) {
                ...
                if ($.aplayer.config[settings.protocol][settings.extension] == 'vlc') {
                    ...
                    if ($.aplayer.config[settings.protocol][settings.extension] == 'embed') {
                        elementMediaType = 'application/' + settings.extension;
                    } else {
                        elementMediaType = $.aplayer.config[settings.protocol][settings.extension] + '/'
                            + settings.extension;
                    }
                }
                $.extend(settings,
                    {mediaType: $.aplayer.config[settings.protocol][settings.extension], 'type': elementMediaType }
                );
            ...
            if ($.aplayer.config[settings.protocol]['*'] != null) {
                if (elementMediaType == null) {
                    if ($.aplayer.config[settings.protocol]['*'] == 'embed') {
                        elementMediaType = 'application/' + ext;
                    } else {
                        elementMediaType = $.aplayer.config[settings.protocol]['*'] + '/' + ext;
                    }
                }
    ...
    play: function (element, settings) {
        ...
        if (sets.height.indexOf('Inherit') != -1) {
            // необходимо не 0 значение для IE для инициализации ActiveX объекта
            sets.height = $element.parents('.pl_wrapper').height();
        }
        if (sets.width.indexOf('Inherit') != -1) {
            // необходимо не 0 значение для IE для инициализации ActiveX объекта
            sets.width = $element.parents('.pl_wrapper').width();
        }
        ...
        var $container = $('<div></div>').attr('id', $.aplayer.idContainer + sets.nr);

        //Установка дополнительного класса
        $container.addClass(settings['class']).addClass('aplayer');

        $element.html($container);
        sets.mediaType = sets.mediaType.toLowerCase();
        ...
        if (sets.logoPlay) {
            ...
            $container
                ...
                .find('.logoPlay').css({
                    'top': (($container.height() - $container.find('.logoPlay').height()) / 2) + 'px'
                });
            ...
            $container
                .click(function () {
                    ...
                    $this.parents('.pl_wrapper').addPlayer({
                        'src': s,
                        'type': '"' + t + '"',
                        'controls': 'mini',
                        'nr': sets.nr
            ...
            $.aplayer.checkCreateElem($container, sets);
        }
    ...
    /**
     * Создает необходимый контейнер $.aplayer'a для медиаэлемента
     * @param container - Контейнер, куда будет помещен медиа элемент
     * @param sets - Параметры медиаэлемента
     */
    checkCreateElem: function (container, sets) {

        if (sets.mediaType.toLowerCase() == 'vlc' && sets.protocol == 'rtsp') {
        ...
        } else if (sets.mediaType.toLowerCase() == 'vlc' && sets.protocol == 'http') {
            $.aplayer.showVLC(container, sets);
        } else if (sets.mediaType == 'video') {
            $.aplayer.IsHtml5Video = true;
            ...
            var typeObj = new Array();
            var typeElem = sets.mediaType.trim();
            if (typeElem.search(',')) {
                typeObj = typeElem.split(',');
                $.aplayer.createEmbeddedPlayers(sets, typeObj, container);
            } else {
    ...
    /**
     * Выбор плеера в зависимости от установленных вариантов
     * @param sets - Настройки aplayer
     * @param typeObj - Массив, содержащий в себе необходимые объекты воспроизведения очереди
     */
    createEmbeddedPlayers: function (sets, typeObj, container) {
        var cont = container;
        var PlugInFind = false;
        for (var i in typeObj) {
            var checkNewType = new checkType(typeObj[i], sets.src);
            if (checkNewType.checkCanPlayTypeFromPlugin()) {
                sets.mediaType = typeObj[i].trim();
                PlugInFind = true;
                break;
            }
        }
    ...
    showImage: function (container, settings) {
        ...
        if (settings.useImageSize != null && settings.useImageSize == 'true') {
            //  title="'+settings.type+'"
            $('<img id="' + $.aplayer.idElMedia + $.aplayer.aplayerNo + '" src="' + settings.src
                + '" class="' + $.aplayer.classElMedia + '" name="img"/>')
                .appendTo(container);
            return;
        }
        ...
        // title="'+settings.type+'"
        var im = $('<img id="' + $.aplayer.idElMedia + $.aplayer.aplayerNo + '" class="'
            + $.aplayer.classElMedia + '" src="' + settings.src + '" ' + size + ' name="img"/>')
            .attr({'height': settings.height, 'width': settings.width });
        $(im).appendTo(container);
    },

    // clsid:"clsid:9BE31822-FDAD-461B-AD51-BE1D1C159921",
    clsid: "clsid:E23FE9C6-778E-49D4-B537-38FCDE4887D8",
    // Create object vlc
    showVLC: function (container, settings) {
        var toolbarShow = true;
        if (settings.src.search('://') === -1) {
            toolbarShow = false;
        }
        var html = '';
        ...
        settings.height = settings.height - $.aplayer.ControlBar.height - 10;

        var Autostart = !!((settings.autostart == "on" || settings.autostart == "true" || settings.autostart == true));

        if ($.browser.msie) {
            html += '<div class="' + $.aplayer.classMediaCont + '" style="overflow:hidden; position:relative; height:'
                + settings.height + 'px; width:' + settings.width + ' ">';
            html += '<OBJECT classid="' + $.aplayer.clsid
            ...
            html += 'id="' + 'activX_' + settings.nr + '" events="True">';
            ...
            html += '<param name="toolbar" value="' + toolbarShow + '" />';
            html += '</OBJECT>';
            html += '</div>'; // classMediaCont

            $(container).html(html);
            $.aplayer.activeX_arr['activX_' + settings.nr] = $('#activX_' + settings.nr, container);

            $('#activX_' + settings.nr, container)
            ...
            html += '<embed type="application/x-vlc-plugin" name="vlc" autoplay="' + Autostart + '" loop="yes" \
				        ...
				        id="' + $.aplayer.embedObj_idname + settings.nr + '" \
            			width="' + settings.width + '" height="' + settings.height + '" \
				        target="' + MediaURL + '" src="' + MediaURL + '" volume="100" toolbar="' + toolbarShow + '">' +
                '</embed>';
    ...
    showMjpeg: function (container, settings) {
        var messageError = $('<div></div>', {
            'class': 'messageError'
        });
        var messageErrorText = $('<div></div>', {
            'class': 'textError'
        });
        $(messageError).append(messageErrorText);
        if ($.browserInfo.browser == 'msie') {
            ...
            if (settings.useImageSize != null && settings.useImageSize == 'true') {
                ...
                var size = 'style="width:' + settings.width + 'px; height:' + settings.height + 'px; "';
                mim = $('<img src="" ' + size + ' name="mjpeg" ' + CORS + ' />')
                    .attr({'height': settings.height, 'width': settings.width });
            }
            $(mim)
                .attr({
                    'id': $.aplayer.idElMedia + $.aplayer.aplayerNo,
                    'class': $.aplayer.classElMedia, 'origsrc': settings.src
                })
                .appendTo(container);
            ...
            if (settings.controls == false || settings.controls == 'off') {
                ...
                $(SubCont).css({'position': 'relative'});
                $(SubCont).append(messageError);
                ...
                var Stop = $($.aplayer.ControlBar.stop)
                    ...
                    .end()
                    .attr({'title': $.aplayer.config.dictionary.stop, 'alt': $.aplayer.config.dictionary.stop });
                ...
                var Play = $($.aplayer.ControlBar.play)
                    ...
                    .end()
                    .attr({'title': $.aplayer.config.dictionary.play, 'alt': $.aplayer.config.dictionary.play });

                //Создаем панель контролов
                var ControlBar = $($.aplayer.ControlBar.panel).height($.aplayer.ControlBar.height);

                $('#' + $.aplayer.idContainer + $.aplayer.aplayerNo)
                    .height($('#' + $.aplayer.idContainer + $.aplayer.aplayerNo)
                        .parent().parent().height());

                meHeight = $('#' + $.aplayer.idContainer + $.aplayer.aplayerNo)
                    .parent().height() - $(ControlBar).height() - 10;
                ...
                var SubCont = $('<div></div>')
                    .addClass($.aplayer.classMediaCont)
                    .css({'overflow': 'hidden'}).height(meHeight);
                $('#' + $.aplayer.idElMedia + $.aplayer.aplayerNo).height(meHeight).appendTo($(SubCont));
                ...
                if (settings.scale == 'on' || settings.scale == 'true' || settings.scale == true) {
                    ...
                    var Scale = $($.aplayer.ControlBar.scale)
                        ...
                        .attr({'title': $.aplayer.config.dictionary.scale, 'alt': $.aplayer.config.dictionary.scale });
    ...
    showPseudo: function (container, settings) {
        ...
        if (settings.useImageSize != null && settings.useImageSize == 'true') {
            ...
            var size = 'style="width:' + settings.width + 'px; height:' + settings.height + 'px; "';
            mim = $('<img  src="' + settings.src + '" ' + size + ' name="pseudo"/>')
                .attr({'height': settings.height, 'width': settings.width });
        }
        ...
        //установка контролов
        if (settings.controls == 'none' || settings.controls == 'false'
            || settings.controls == false || settings.controls == 'off') {
        } else {
            var Stop = $($.aplayer.ControlBar.stop)
                .css($.aplayer.ControlBar.ControlsContainers)
                .css({'padding': '5px'})
    ...
    showObject: function (container, settings) {
        ...
        if (settings.protocol == 'rtsp') {
            ...
            return;
        } else if (settings.type.indexOf($.aplayer.MIMEtypes.mp4) != -1
            || settings.type.indexOf($.aplayer.MIMEtypes.mov) != -1) {
            obj = $.aplayer.createObj_QuickTime(settings);
        } else if (settings.type.indexOf($.aplayer.MIMEtypes.swf) != -1
            || settings.type.indexOf($.aplayer.MIMEtypes.flv) != -1) { //SWF
            obj = $.aplayer.createObj_SWF(settings);
        } else if (settings.type.indexOf($.aplayer.MIMEtypes.wmv) != -1) { //wmv - в IE глюки
            obj = $.aplayer.createObj_WMV(settings);
        } else if (settings.type.indexOf($.aplayer.MIMEtypes.mp3) != -1) { // audio/mp3
            obj = $.aplayer.createObj_MP3(settings);
        } else if (settings.type.indexOf($.aplayer.MIMEtypes.wav) != -1) { // audio/wav
            obj = $.aplayer.createObj_WAV(settings);
        } else if (settings.type.indexOf($.aplayer.MIMEtypes.avi) != -1) { // video AVI
...
//				.append('<param name="play" value="true" >');
    ...
    showVideo: function (container, settings) {
        ...
        $(vid).attr({
            ...
            src: [settings.src],
            type: [settings.type],
            id: [$.aplayer.idElMedia + settings.nr],
            'class': $.aplayer.classElMedia,
            'preload': 'auto'
        }).appendTo(container);
        $.aplayer.setControls(container, settings);
    ...
    ControlBar: {
        ...
        elMediaOnTimeUpdate: function (ElNum) {
            if ($('#' + $.aplayer.idSearch + ElNum).attr('isPlaying') != undefined) {
                return;
            }
            var MediaElt = $('#' + $.aplayer.idElMedia + ElNum)[0];
            if (MediaElt == undefined) {
                return;
            }
            ...
            if ($('#' + $.aplayer.idContainer + ElNum).aplayerIsHTML5_Video() ||
                $('#' + $.aplayer.idContainer + ElNum).aplayerIsHTML5_Audio()) {
                var newTime = $.aplayer.ControlBar.FormatTimeReally(tDur, ElNum);
            } else {
                var newTime = new Array(tStr, tDur);
            }

            $('#' + $.aplayer.idCurrentTime + ElNum).html(newTime[0]);
            $('#' + $.aplayer.idDuration + ElNum).html(newTime[1]);
        },
...
//						console.log(error.message);
        ...
        searchOnStartHandler: function (Elem) {
            ...
            if (!me.paused) {
                ...
                $(Elem).attr({'isPlaying': 'false'});
            }
            me.pause();
        },

        getDurationElement: function (Elem) {
            var MediaElt = $('#' + $.aplayer.idElMedia + Elem)[0];
            if (MediaElt == undefined) {
                return '00:00:00';
            }
            return $.aplayer.ControlBar.FormatTime(Math.round(MediaElt.duration));

        },

        searchOnStopHandler: function (Elem) {
            var me = $('#' + $.aplayer.idElMedia + $(Elem).attr('No'))[0];
            var numElMedia = me.id.substr($.aplayer.idElMedia.length);
            $(me).each(function () {
                ...
                if ($.aplayer.IsHtml5Video) {
                    var newTime = $.aplayer.ControlBar.FormatTimeReally(setTime, numElMedia);
                } else {
                    var newTime = new Array(tStr, $.aplayer.getDurationElement(numElMedia));
                }

                $('#' + $.aplayer.idCurrentTime + $(Elem).attr('No')).html(newTime[0]);
                $('#' + $.aplayer.idDuration + Elem).html(newTime[1]);
            });
            $(me).each(function () {
        ...
        searchOnMousemoveHandler: function (Elem, target_value) {
            var me = $('#' + $.aplayer.idElMedia + $(Elem).attr('No'))[0];
            if (me.seekable.length<1) {
                return;
            };
            var s=false;
            for (i=0; i<me.seekable.length && !s; i++) {
                s=s || (target_value/10>=me.seekable.start(i) && target_value/10<=me.seekable.end(i));
            };
            if (!s) {
                return;
            };
            var numElMedia = me.id.substr($.aplayer.idElMedia.length);
            var newTime = $.aplayer.ControlBar.FormatTimeReally(target_value / 10, numElMedia);
            return newTime[0];
        },
        ...
        FormatTimeReally: function (tCur, ElNum) {
            if ($('#aplayerNo_' + ElNum).size() == 0) {
                return null;
            }
            var cel_nr = parseInt($('#aplayerNo_' + ElNum).parents('.content_item').attr('id').replace('cell_', ''));
            var tempDate = '0000-00-00 00:00:00';
            if (matrix != undefined) {
                tempDate = matrix.events[cel_nr][9]
            }
            var dt = tempDate;
            var dateObj = new Date(dt.replace(/(\d+)-(\d+)-(\d+)/, '$2/$3/$1'));
            dateObj.setSeconds(dateObj.getSeconds() + Math.round(tCur));

            var nDate = tempDate.replace(new RegExp("-", 'g'), "/").split(' ');
    ...
    setControls: function (container, settings) {
        var time = $.aplayer.ControlBar.FormatTimeReally(0, settings.nr);
        if (typeof(settings.controls) != 'undefined' && (settings.controls == 'none' || settings.controls == 'false' || settings.controls == false || settings.controls == 'off')) {
            return;
        } else if (settings.controls == 'browser') { //Использовать контролы браузера
        ...
        } else if (settings.mediaType.toLowerCase() === 'vlc' && typeof(vlcPlayer) !== 'undefined') {
            vlcPlayer.registerVLCEvent('MediaPlayerSeekableChanged',function(e) {
                $('.ui-slider',container).attr('seekable',e?'true':'false');
            },settings.nr);
            evt_handler = vlcPlayer;
            str_evt_handler = 'vlcPlayer';
        } else {
        ...
        var Volume = $($.aplayer.ControlBar.volume)
            .css($.aplayer.ControlBar.ControlsContainers)
            .attr({'title': $.aplayer.config.dictionary.volume});
        ...
        $(Volume).find('.divSlider').slider({
            ...
            slide: function (event, ui) {
                evt_handler.volumeSlideHandler(this);
            },
        ...
        var soundOn = $($.aplayer.ControlBar.soundOn)
            ...
            .end()
            .attr({'title': $.aplayer.config.dictionary.soundOn, 'alt': $.aplayer.config.dictionary.soundOn});
        ...
        var soundOff = $($.aplayer.ControlBar.soundOff)
            ...
            .end()
            .attr({'title': $.aplayer.config.dictionary.soundOff, 'alt': $.aplayer.config.dictionary.soundOff });
        var CurrentTime = $('<div>' + time[0] + '</div>')
            .attr({'id': $.aplayer.idCurrentTime + settings.nr, 'title': $.aplayer.config.dictionary.currentTime})
            .css($.aplayer.ControlBar.TimeFontStyle);

        var Duration = $('<div>' + time[1] + '</div>')
            .attr({'id': $.aplayer.idDuration + settings.nr, 'title': $.aplayer.config.dictionary.duration})
            .css($.aplayer.ControlBar.TimeFontStyle);

        var Times = $('<div />').append(Duration).append(CurrentTime);

        if (typeof matrix.mode !== 'undefined' && matrix.mode == 'preview') {
            $(Duration).hide()
        } else {
            $(Duration).show();
        }
        var Search = $($.aplayer.ControlBar.search).attr({'title': $.aplayer.config.dictionary.unseekable});
        $(Search).find(':first-child').slider({
            range: "min",
            value: 0,
            start: function (event, ui) {
                evt_handler.searchOnStartHandler(this);
            },
            stop: function (event, ui) {
                evt_handler.searchOnStopHandler(this);
            },
            slide: function (event, ui) {
                evt_handler.searchOnSlideHandler(this);
            }
        }).attr({
            ...
            .find('.ui-slider-range')
            .addClass('ui-corner-all')
            .attr({'left': '2px', 'width': '99.25%'});

        $('.ui-slider',Search).mousemove(function(event) {
            x=event.pageX-$(this).parent().offset().left;
            a=$(this).slider('option','max');
            i=$(this).slider('option','min');
            v=Math.round(i+(a-i)*x/$(this).outerWidth())
            t=evt_handler.searchOnMousemoveHandler(this,v);
            if (typeof t === 'undefined') {
                t=$.aplayer.config.dictionary.unseekable;
                $('.ui-slider',Search).css('cursor','not-allowed');
            } else {
                $('.ui-slider',Search).css('cursor','pointer');
            };
            Search.attr('title',t);
        });
        ...
        var Pause = $($.aplayer.ControlBar.pause)
            ...
            .end()
            .attr({'title': $.aplayer.config.dictionary.pause, 'alt': $.aplayer.config.dictionary.pause });
        ...
        if (settings.mediaType == 'flowplayer' && typeof(fp_uicontrol_handler) != 'undefined') {
            ...
            //если не flowplayer Создаем субконтейнер для медиа-элемента, вставлем в него медиа элемент и помещаем
            // в контейнер плеера
            var SubCont = $('<div style="overflow:hidden; position:relative;"></div>')
                .addClass($.aplayer.classMediaCont)
                .css({'overflow': 'hidden'})
            ...
            if ($('#' + $.aplayer.idElMedia + settings.nr).attr('id') !== undefined) {
                $('#' + $.aplayer.idElMedia + settings.nr)
                    .height(meHeight)
                    .appendTo($(SubCont));
            } else {//VLC
                $('#' + $.aplayer.embedObj_idname + settings.nr)
                    .height(meHeight)
                    .appendTo($(SubCont));
            }
            if ($.browser.msie && $('#' + $.aplayer.idContainer + settings.nr).aplayerIsVlc()) {
                $('#' + $.aplayer.idContainer + settings.nr).append($(ControlBar));
            } else {
        ...
        if ($('#' + $.aplayer.idContainer + settings.nr).aplayerIsFlowPlayer()) {
            $f($.aplayer.idMediaCont + settings.nr)
                .onStart(function () {
                    fp_uicontrol_handler.elMediaOnPlay(settings.nr);
                })
                .onStop(function () {
                    fp_uicontrol_handler.elMediaOnStop(settings.nr);
                })
                .onPause(function () {
                    fp_uicontrol_handler.elMediaOnPause(settings.nr);
                })
                .onResume(function () {
                    fp_uicontrol_handler.elMediaOnResume(settings.nr);
                })
                .onFinish(function () {
                    fp_uicontrol_handler.elMediaOnFinish(settings.nr);
                })
                .onSeek(function () {
                    fp_uicontrol_handler.elMediaOnSeek(settings.nr);
                })
                .onBeforeSeek(function () {
                    fp_uicontrol_handler.elMediaOnBeforeSeek(settings.nr);
                })
        }
    }
};
...
$.browserInfo = {
    ...
    //Check HTML5_Video
    support_HTML5_Video: function (src) {
    ...
    support_video_mp4: function (video) {
        ...
        if ($.browserInfo.video_mp4 == '') {
            $.browserInfo.video_mp4 = video.canPlayType('video/mp4');
        }
        if ($.browserInfo.video_mp4 == '') {
            $.browserInfo.video_mp4 = false;
        }
    },
    ...
    support_video_ogg: function (video) {
        ...
        if ($.browserInfo.video_ogg == '') {
            $.browserInfo.video_ogg = video.canPlayType('video/ogg;');
        }
        if ($.browserInfo.video_ogg == '') {
            $.browserInfo.video_ogg = false;
        }
    },
    ...
    support_video_webm: function (video) {
        ...
        if ($.browserInfo.video_webm == '') {
            $.browserInfo.video_webm = video.canPlayType('video/webm;');
        }
        if ($.browserInfo.video_webm == '') {
            $.browserInfo.video_webm = false;
        }
    },

    support_audio_ogg: function (audio) {
        $.browserInfo.audio_ogg = audio.canPlayType('audio/ogg; codecs="vorbis"');
        if ($.browserInfo.audio_ogg == '') {
            $.browserInfo.audio_ogg = audio.canPlayType('audio/ogg');
        }
        if ($.browserInfo.audio_ogg == '') {
            $.browserInfo.audio_ogg = false;
        }
    },
    support_audio_mpeg: function (audio) {
        $.browserInfo.audio_mpeg = audio.canPlayType('audio/mpeg;');
        if ($.browserInfo.audio_mpeg == '') {
            $.browserInfo.audio_mpeg = audio.canPlayType('audio/mpeg;');
        }
        if ($.browserInfo.audio_mpeg == '') {
            $.browserInfo.audio_mpeg = false;
        }
    },
    support_audio_wav: function (audio) {
        $.browserInfo.audio_wav = audio.canPlayType('audio/wav; codecs="1"');
        if ($.browserInfo.audio_wav == '') {
            $.browserInfo.audio_wav = audio.canPlayType('audio/wav');
        }
        if ($.browserInfo.audio_wav == '') {
            $.browserInfo.audio_wav = false;
        }
    },
    support_audio_x_m4a: function (audio) {
        $.browserInfo.audio_x_m4a = audio.canPlayType('audio/x-m4a;');
        if ($.browserInfo.audio_x_m4a == '') {
            $.browserInfo.audio_x_m4a = audio.canPlayType('audio/aac;');
        }
        if ($.browserInfo.audio_x_m4a == '') {
            $.browserInfo.audio_x_m4a = false;
        }
    }
