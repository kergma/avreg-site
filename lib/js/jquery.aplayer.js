	...
	$.aplayer = {
		...
		stopPlay : function(aplayerID){
			...
			if($('#'+$.aplayer.embedObj_idname+pl_nr).attr('id')!=null ){
				javascript:document[$.aplayer.embedObj_idname+pl_nr].Stop();
			
			//если объект - прячем его, а вместо показываем постер "остановлено" 
			$('<img class="pl_poster" src="'+$.aplayer.ControlBar.controlsImg+'Stoped.png'+'">')
		...
		startPlay : function(aplayerID){
			...
			if($('#'+$.aplayer.embedObj_idname+pl_nr).attr('id')!=null ){
				...
				javascript:document[$.aplayer.embedObj_idname+pl_nr].Play();
				return;
			}
...
//			$.aplayer.config.Show();
		...
		versionPredicate : function(f,s){
			...
			setType : function(settings){
				...
				if(settings.mediaType == 'pseudo') return settings;
...
//						$('.'+$.aplayer.classMediaCont, container).unbind('mousewheel');
			...
			stopMjpegPlay : function(mim){
				$(mim).attr({'src': $.aplayer.ControlBar.controlsImg+'Stoped.png'});
...
//				$(obj).append('<param name="allowScriptAccess" value="sameDomain" />');
            ...
            createObj_WMV:function(settings){
			 ...
			 $(obj).append('<param name="wmode" value="window" >')
				...
				$(obj).width(settings.width)
                .height(settings.height);
			   $(obj).append('<embed type="audio/wav" play="'+Autostart+'" wmode="window" PLUGINSPAGE="http://www.microsoft.com/windows/windowsmedia/download/" src="'+settings.src+'" style="width:'+settings.width+'px; height:'+settings.height+'px;" autostart="'+Autostart+'" showcontrols="true"></embed>');
               return obj;
            },
            ...
            createObj_RTSP : function(settings){
            	...
            	if($.browser.msie){
            		...
            		var feed = document.createElement('object');
                    //-->
            		// feed.setAttribute("feedtype", streamType, 1);
                    
                    feed.id = 'activX_'+$.aplayer.aplayerNo;
                    feed.width = settings.width;
                    feed.height = settings.height;
                    feed.setAttribute("codebase", "http://www.apple.com/qtactivex/qtplugin.cab");        
                    feed.setAttribute("classid", "clsid:02BF25D5-8C17-4B23-BC80-D3488ABDDC6B");
                   
                    var paramSRC = document.createElement('param');
                    paramSRC.name = "src";
                    paramSRC.value = rtsptextrtsp;
                    feed.appendChild(paramSRC);
                   
                    var paramAutoplay = document.createElement('param');
                    paramAutoplay.name = "autoplay";
                    paramAutoplay.value = "true";
                    feed.appendChild(paramAutoplay);
                   
                    var paramController = document.createElement('param');
                    paramController.name = "controller";
                    paramController.value = (settings.controls == false)? 'false':'true';
                    paramController.value = 'true';
                    feed.appendChild(paramController);
                   
                    var paramQTSRC = document.createElement('param');
                    paramQTSRC.name = "qtsrc";
                    paramQTSRC.value = MediaURL;
                    feed.appendChild(paramQTSRC);
                   
//                    
//                    
//                    
//                    var embed = document.createElement('embed');
//                    embed.src = rtsptextrtsp;
//                    embed.width = settings.width;
//                    embed.height = settings.height;
//                    embed.setAttribute("qtsrc", MediaURL);
//                    embed.setAttribute("autoplay", "true");
////                    embed.setAttribute("controller", (settings.controls == false)? 'false':'true');
//                    embed.setAttribute("target", "myself");
//                    embed.setAttribute("pluginspace", "http://www.apple.com/quicktime/download/");
// // НЕ АППЕНДИТ embed !!!!                   
////                    feed.appendChild(embed);
		...
		setControls : function(container, settings)
		{
			...
			if(settings.controls=='none' || settings.controls=='false' || settings.controls==false || settings.controls!='off') return;
