...
//        }catch(e){}
     ...
     * @returns {number} -
     */
    searchTimeSlider: function (numPlayer,slider_value) {
        var len = (vlcPlayer.retLen(numPlayer) > 0) ? vlcPlayer.retLen(numPlayer) : vlcPlayer.retDuration(matrix.events[numPlayer][8]);
        var step = len / 1000 / 100;
        if (typeof slider_value==='undefined') {
            slider_value=$('#' + $.aplayer.idSearch + numPlayer).slider('value');
        };
        return slider_value * step;
    },

    searchOnMousemoveHandler: function (container,target_value) {
        var numPlayer = $(container).attr('no');
        if ($(container).attr('seekable')!='true') {
            return;
        };
        var time = vlcPlayer.searchTimeSlider(numPlayer,target_value);
        return $.aplayer.ControlBar.FormatTimeReally(time, numPlayer)[0];
    },
