...
var user_layouts = {
    ...
    to_tune_mode: function (layout_id, redirect_url) {
        var self = this;
    ...
    tune_save: function (layout_id, redirect_url) {
        var self = this;
        //данные раскладки
        var layout_data = {};
        ...
        //Сохранить?
        confirm("Сохранить изменения пользовательской раскладки № " + layout_id + " ?");

        //номер раскладки
        //layout_data.MON_NR = parseInt($('input[name=mon_nr]').attr('value'));
        var MON_NR = layout_id;
        ...
        //по умолчанию?
        layout_data.d = this.client_layouts[MON_NR].d;
    ...
    remove: function (layout_id, redirect_url) {
        var self = this;

        confirm("Удалить пользовательскую раскладку № " + layout_id + " ?");

        try {
            //Удаляем раскладку
            delete this.client_layouts[layout_id];
        } catch (e) {
            alert('Not delete layouts');
        }
        ...
        if (objlen == 0) {
            ...
            var json = undefined;
            this.setCookie('layouts', json_str, '', '/', window.location.hostname + WwwPrefix, '');
        } else {
            var json_str = JSON.stringify(this.client_layouts);
            self.client_layouts_json = json_str;
            self.local_storage.setItem(self.storage_name, json_str);
...
// 		var MON_NR = parseInt($('input[name=mon_nr]').attr('value'));
        ...
        var MON_NR = $('input[name=mon_nr]').attr('value');
        ...
        //переадресуем на онлайн просмотр
        self.redirect('../online/view.php', true);
    ...
    redirect: function (url, layouts) {
        ...
        } else if (layouts && user_layouts.isLocalStorageAvailable() && user_layouts.client_layouts_json) {
            //Передаем параметры пользовательских раскладок
            if (url.indexOf('?') == -1) {
                this.setCookie(
                    'layouts',
                    JSON.stringify(user_layouts.client_layouts), 86400, '/', window.location.hostname, ''
                );
            } else {
                this.setCookie(
                    'layouts',
    ...
    setUserLayoutsDefault: function (number_of_layouts) {
        $(document).find('[noLay^=' + number_of_layouts + ']').each(function () {
        ...
        this.setCookie('layouts', JSON.stringify(user_lay), 86400, '/', window.location.hostname, '');
    },
    ...
    setCookie: function (name, value, expires, path, domain, secure) {
        ...
        expires = isNaN(parseInt(expires)) ? 0 : parseInt(expires);
        var dat_exp = new Date();
        dat_exp.setMinutes(dat_exp.getMinutes(), dat_exp.getSeconds() + expires);
        if (expires) {
            str += '; expires=' + dat_exp.toGMTString();
        }
        if (path) {
            str += '; path=' + path;
        }
