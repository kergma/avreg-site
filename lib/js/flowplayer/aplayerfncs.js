var fp_uicontrol_handler = {
    ...
    updateCurTime: function (ElNum) {
        ...
        if ($('#' + $.aplayer.idMediaCont + ElNum).length == 0) {
    ...
    elMediaOnStop: function (ElNum) {
        ...
        var tCurStr = fp_uicontrol_handler.FormatTimeReally(0, ElNum);
        $('#' + $.aplayer.idCurrentTime + ElNum).html(tCurStr[0]);
    ...
    searchOnStopHandler: function (Elem) {
        ...
        if (player.getStatus()['bufferEnd'] == 0) {
            //возвращаем слайдер на начальную позицию
            $('#' + $.aplayer.idSearch + ElNum).slider('value', 0);
            return;
        }
        var setTime = Math.round(($(Elem).slider('value')) / 30);
        var tCurStr = fp_uicontrol_handler.FormatTimeReally(setTime, ElNum);
        $('#' + $.aplayer.idCurrentTime + ElNum).html(tCurStr[0]);
    ...
    searchOnMousemoveHandler: function (Elem,target_value) {
        var ElNum = $(Elem).attr('No');
        var player = $f($.aplayer.idMediaCont + ElNum);
        if (typeof player.seekable !== 'undefined' && !player.seekable) {
            return;
        };
        var tCurStr = fp_uicontrol_handler.FormatTimeReally(Math.round(target_value / 30), ElNum);
        return tCurStr[0];
    },
