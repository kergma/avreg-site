...
$source_absent_warn = 'Прим.: запись начнётся только когда будут захвачены все медиа-потоки, определённые в {video_src}/{audio_src}. Если какой-любо из определённых источников не может быть получен - запись не начнётся.';

$event_groups = array( 'mediafiles' => 'медиафайлы',
                       'snapshots'  => 'картинки',
                       'capture'    => 'захват',
                       'motion'     => 'движение',
                       'quality'    => 'качество',
...
$PARAMS = array(
   ...
   array(
      ...
      'type'    => $BOOL_VAL,
      'def_val' => TRUE,
      'desc'    => '<b>Декодировать входящий видеопоток (<span class="warntext">не допускайте увеличения %CPU более 80% на каждом процессорном ядре</span>)</b>.
      ...
      <li>перекодирование для записи, когда кодек записи, заданный явно (см. {rec_vcodec}) или стандартный для выбранного формата (см. {rec_format}), не совпадает с кодеком исходного входящего видеопотока,</li>
      <li>локальное наблюдение с помощью программы avreg-mon (Наблюдение -&gt; локальное),</li>
      <li>наблюдение по сети со сжатием MJPEG (прим. AVReg v5.6/6 по другому не умеет), а исходный входящий поток не MJPEG.</li>
      </ul>
      <p>При использовании мультипоточных ip-камер и нехватки ресурсов сервера планируйте <a href="http://avreg.net/manual_applications_multi-stream.html" target="_blank">эффективные решения</a>.<p>
   ...
   array(
      ...
      <p>Декодирование необходимо если необходимо перекодирование для записи, когда кодек записи, заданный явно (см. {rec_acodec}) или стандартный для выбранного формата (см. {rec_format}), не совпадает с кодеком исходного входящего аудиопотока.</p>
По умолчанию: &quot;пусто&quot; - декодировать.',
      'flags'   => $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
      'cats'    => '3',
      'subcats' => '3.1;3.2',
   ...
   array(
      ...
      <ul>
      <li>Для <b>сетевых ip-камер</b> и ip-видеосерверов установите действительное разрешение видеопотока, которые определены в настройках самой камеры или заданы в параметрах запроса ({rtsp_play}/{V.http_get}) к камере.</li>
      <li>Для <b>video4linux</b> устройств установите одно из поддерживаемых драйвером устройства значение. Список поддерживаемых разрешений можно посмотреть в выводе команды <span class="cmd">v4l2-ctl --list-formats-ext -d /dev/videoX</span>, где v4l2-ctl - утилита из пакета v4l-utils, а X - номер устройства. Если драйвер не имеет списка поддерживаемых разрешений, то выберете одно из стандартных разрешений: 384x288<sup>*</sup>, 480x360<sup>*</sup>, 560x420<sup>*</sup>, 640x480<sup>*</sup>, 720x540<sup>*</sup>, 720x576 (макс.&nbsp;saa713x), 768x576<sup>*</sup> (макс.&nbsp;bt878a). Прим:. для ВСЕХ каналов ОДНОГО устройства видеозахвата (одного {v4l_dev}) должно быть ОДНО значение</li>
      ...
      array(
         ...
         Примечание:
         <ul>
         <li>потоковым способом (<b>motion jpeg</b>) - настройка скорости в кадрах в секунду (framerate) регулируется (ограничивается) <b>только настройками самих ip-камер/видеосерверов</b>, а в продвинутых моделях может указываться в параметрах запроса к устройствам (например, <nobr>V.http_get=/axis-cgi/mjpg/video.cgi?<b>fps=15</b></nobr>);</li>
         </ul>
...
array(
   ...
   <p><b>Сочетание значений v4l_dev и input</b> фактически <b>указывают на номер разъёма</b>. Определить состав каналов поможет утилита<span class="cmd">v4l2-ctl --list-inputs -d /dev/videoX</span>, где v4l2-ctl - утилита из пакета v4l-utils, а X - номер устройства или любая ТВ-смотрелка (xawtv, tvtime, ...).
...
array(
   ...
   <li><b>FOURCC тэг</b> - 4 символа, обозначаюшие формат изображения. Например, &quot;YUYV&quot; (YUV 4:2:2) или &quot;MJPG&quot;, &quot;JPEG&quot;, &quot;MPEG&quot; (если поддерживаются). Посмотреть какие FOURCC форматы поддерживает ваша плата видеозахата или USB-камера /dev/videoX можно в выводе команды <span class="cmd">v4l2-ctl --list-formats -d /dev/videoX</span>, где v4l2-ctl - утилита из пакета v4l-utils, а X - номер устройства.
   </li>
...
array(
   ...
   <li><b>[1-60] или дробь период_в_сек/кадров_за_период</b>  - ограничить скорость видеозахвата. Если драйвер устройства поддерживает управление скоростью захвата, то необходимо выбирать значения fps поддерживаемые устройством/драйвером: см. вывод команды <span class="cmd">v4l2-ctl --list-formats-ext -d /dev/videoX</span>, где v4l2-ctl - утилита из пакета v4l-utils, а X - номер устройства.</li>
   </ul>
   Примечания:
   <ul>
   <li>при использовании PCI плат видеозахвата без режима мультиплексирования (каждая  камера подключена на свой отдельный видеокодер) или USB-камер, <i>максимальная скорость видеозахвата</i> ограничивается используемым телевизионным стандартом (25fps PAL, 30fps NTSC) и возможностями устройства (если USB-камера).</li>
...
array(
   ...
   'type'    => $INT_VAL,
   'def_val' => NULL,
   'desc'    => '<b>Яркость</b> ' . $v4l_int_cntrl . ' <br><br>Прим.: другие многочисленные (обычно менее применимые) video4linux-параметры можно устанавливать до запуска демона &#171;'.$conf['daemon-name'].'&#187;, определяя параметры запуска модулей ядра устройств видеозахвата (см. MODINFO(8) и MODPROBE.CONF(5)) или с помощью специальных video4linux утилит. Например, для просмотра всех параметров устройства выполните в терминале <span class="cmd">v4l2-ctl --list-ctrls --list-ctrls-menu -d /dev/videoX</span>, где v4l2-ctl - утилита из пакета v4l-utils, а X - номер устройства.',
   'flags'=>$F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
   'cats'    => '3.2',
   'subcats' => NULL,
   'mstatus' => 1,
...
array(
   ...
   'type'    => $STRING_VAL,
   'def_val' => NULL,
   'desc'    => '<b>Список камер, чья запись должна управляться детектором этой камеры</b>. Действует только на камеры с:
   <ul>
   ...
   <li><span class="warntext">из того же <a href="http://avreg.net/manual_applications_smp.html" targer="_blank">профиля (процесса avregd)</a> что и эта камера</span>.</li>
   </ul>
   <p>Пример списка: &quot;5&quot; или &quot;2-5, 11, 20&quot; (указывать без кавычек).<br>По умолчанию: &quot;пусто&quot; - управлять записью только своей камеры.</p>',
   'flags'   => $F_RELOADED | $F_IN_CAM,
   'cats'    => '5.1.3',
   'subcats' => NULL,
   'mstatus' => 1,
...
array(
   ...
   'type'    => $INT_VAL,
   'def_val' => 10,
   'desc'    => '<b>Период &quot;спокойствия&quot; детектора движения в секундах, логически отделяющий один сеанс движения от другого</b>.
   <br><br>При выборочном режиме записи </nobr>({rec_mode} = &quot;Выборочно&quot;)</nobr>, запись файла(ов) на диск производится только внутри сеанса. В общем случае, увеличение значения {motion_session_end} приводит к увеличению объёма записи при прочих равных условиях.
...
array(
   ...
   <div class="warn">
   Если вы планируете явно выбрать формат из списка то не исключайте:
   <ul>
   <li>блокировку процесса записи, например, при невозможности перекодирования (отсутствия встроенных кодеков сжатия);</li>
   <li>значительно возросшую нагрузку на CPU (из-за декодирования с последующим кодированием), следите чтобы она не превышала 80%.</li>
   </ul>
   </div>
...
array(
   ...
   'type'    => $INT_VAL,
   'def_val' => 3,
   'desc'    => '<b>Буфер предзаписи в секундах.</b><p><b>Серьёзно увеличивает требование к объёму оперативной памяти</b></p>Допустимые значения: [1..5]. По умолчанию: <b>1 сек.</b>.',
...
array(
   ...
   'type'    => $INT_VAL,
   'def_val' => 5,
   'desc'    => '<b>Степень сжатия кодируемых (если используется перекодировка) медиапотоков</b> или некий коэффициент, <b>обратный  качеству изображения</b>. Параметр соответствует ffmpeg-параметру qscale для режима VBR (переменный битрейт с целью удержания постоянного качества изображения/звука.<br><br>Прим.: <b>не используется при записи в оригинальном формате захвата</b>.<br><br>Допустимые значения: <b>2</b>(лучшее качество и макс. размер файла) - <b>30</b>(хуже, но меньше).<br/>По умолчанию <b>5</b>(оптимально).',
   'flags'   => $F_RELOADED | $F_IN_DEF | $F_IN_CAM,
   'cats'    => '11',
   'subcats' => NULL,
   'mstatus' => 1,
...
array(
   ...
   'type'    => $INT_VAL,
   'def_val' => 0,
   'desc'    => 'Ограничивает скорость потока &quot;cпокойных&quot; кадров в отсутствии движения до установленного вами значения, естественно, только при включенном детекторе движения. <br>Допустимые значения: [1..60]; по-умолчанию: 60 - не ограничивать.',
   'flags'   => $F_RELOADED | $F_IN_DEF | $F_IN_CAM,
   'cats'    => '15.1',
   'subcats' => NULL,
   'mstatus' => 1,
...
array(
   ...
   'type'    => $STRING200_VAL,
   'def_val' => NULL,
   'desc'    => '<b>Прямой URL веб-интерфейса ip-камеры</b> (указывать с префиксом протокола, http:// или https://).
   <br><br>Значение этого параметра будет подставленно в виде ссылки в заголовке окна камеры на странице просмотра в реальном времени для пользователей все групп, исключая группу &#171;Только просмотр&#187;. Ссылка предназначена для прямого перехода в &quot;родной&quot; веб-интерфейс сетевой камеры (откроется в новом окне) со страницы просмотра в реальном времени веб-интерфейса AVReg.
   <br><br>По умолчанию: не установлено - URL будет построен как &quot;<b>http://{InetCAM_IP}:{InetCam_http_port}</b>&quot;, где параметры в фигурных скобках - параметры захвата с сетевых камер (см. раздел Захват - по сети).',
   'flags'   => $F_RELOADED | $F_IN_CAM,
   'cats'    => '15.2',
   'subcats' => NULL,
   'mstatus' => 1,
...
array(
   ...
   'type'    => $STRING200_VAL,
   'def_val' => NULL,
   'desc'    => '<b>Альтернативные источники видео (минуя avregd)</b> для конфигурирования раскладок просмотра веб-браузерами.
   <p>Альтернативный <b>URL #1</b> для камеры в раскладке, например, для камер Axis: rtsp://login:password@axis-camera-ip/axis-media/media.amp?resolution=320x240&videocodec=h264&audio=0</p>
   <p>Определив эти альтернативные url-лы, в настройках веб-раскладки для этой камеры в качестве источника видеоданных вместо &quot;avregd&quot; нужно выбрать &quot;ALT1&quot; или &quot;ALT2&quot;.</p>
   <div>Примечания:
   <ul>
   <li>Для RTSP url-ов на клиенте потребуется <a href="http://avreg.net/howto-install-vlc-plugin.html" target="_blank">установить плагин VLC Browser Plugin</a>.</li>
...
array(
   ...
   'desc'    => '<b>Группы событий для записи в базу данных в таблицу <a href="http://avreg.net/manual_applications_avreg5_db-structure.html" target="blank">EVENTS</a></b>.
   <br><br>Основное назначение этого параметра - сократить размер базы данных, исключив ненужные для последующего анализа данные:
   <ul>
   ...
   <li>запись - команды на включение или выключение режима записи детектором движения и/или командой пользователя (не путать с событием сохранения файла в архиве),</li>
   <li>клиенты - подключение/отключение сетевых клиентов.</li>
   </ul>
   <b>Примечание</b>:
   <ol>
   <li>отключение групп &#171;медиафайлы&#187; и &#171;картинки&#187; сделает соответствующие файлы невидимыми для чистильщика архива от старых записей и на страницах веб-интерфейса.</li>
   <li>см. также описание опции командой строки и конфигурационного файла &#171;db-disable-events&#187; <span class="cmd">man avregd</span>.</li>
...
array(
   ...
   'desc'    => '<b>Группы событий, передаваемые по каналу PIPE(7) во внешний скрипт &#171;<span class="param">event-collector</span>&#187;</b>.
   <p>
   Под скриптом понимается любое исполняемое приложение, которое может быть написано вами на любом языке программирования (shell, python, perl, ruby, php, java и даже на С). Пример на языке shell см. в каталоге &#171;<span class="path">/usr/share/doc/avregd/examples/</span>&#187;
   </p>
   <p>
   Скрипт должен быть скопирован в каталог &#171;<span class="path">/etc/avreg/scripts/</span>&#187; и его абсолютный путь прописан в конфигурационном файле &#171;<span class="path">avreg.conf</span>&#187; в значении параметра &#171;<span class="param">event-collector</span>&#187; секции параметров демона <nobr>avregd { ... }</nobr>.
   </p>
