...
array(
  ...
  'type'    => $BOOL_VAL,
  'def_val' => true,
  'desc'    => 'Какие видеокадры записывать <b>внутри каждой серии движения</b>:
<ul>
...
array(
  ...
  'type'    => $INT_VAL,
  'def_val' => 0,
  'desc'    => '<em>Длительность в секундах 1-го спапшотного периода</em> с момента начала выборочной записи (например, с начала каждого сеанса движения; см. описание параметра <em>save_video</em>), в котором (периоде) на диск со скоростью не более чем 1 fps (<em>1 кадр в секунду</em>) сохраняются кадры-отметки в формате jpeg, дополнительно к записи в основной формат (см. описание <em>V.save_fmt</em> выше). На диск записываются любые кадры,  как &quot;c движением&quot; так и &quot;спокойные&quot;, т.е. без учёта детектора движения.
...
array(
  ...
  'type'    => $INT_VAL,
  'def_val' => 0,
  'desc'    => '<em>Длительность в секундах 2-го спапшотного периода</em> с момента окончания 1-го периода (см. <em>extra_snap_period1</em> выше), в котором (периоде) на диск со скоростью не более чем 0.2 fps (<em>1 кадр каждые 5 секунд</em>) сохраняются кадры-отметки в формате jpeg, дополнительно к записи в основной формат (см. описание <em>V.save_fmt</em> выше). На диск записываются любые кадры,  как &quot;c движением&quot; так и &quot;спокойные&quot;, т.е. без учёта детектора движения.
...
array(
  ...
  'type'    => $INT_VAL,
  'def_val' => 60,
  'desc'    => 'Ограничить этим значением скорость отдаваемого видеопотока, в кадрах в секунду.<br><br>По умолчанию: <b>60 - не ограничивать</b>.',
  'flags'   => $F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '15.2',
  'subcats' => NULL,
  'mstatus' => 2,
