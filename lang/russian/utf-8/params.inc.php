...
<p>Обычно video4linux устройство указывает на свой конкретный видеодекодер BT878/SAA71xx/CX2388x, которых на одной плате может быть и несколько. Например, большинство 16 канальных плат с 4 видеодекодерами будут представлены в системе как 4 отдельных устройства /dev/video[0..3]. Встречаются и исключения, например, плата Kodikom 4400R, которая представлена драйвером как одно 16-канальное video4linux устройство.</p>По умолчанию: <b>не установлено</b>.',
  'flags'=>$F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '3.2',
  'subcats' => NULL,
  'mstatus' => 2,
...
<p><b>Скорость захвата с мультиплексируемых камер</b> (к одному видеодекодеру BT878/SAA71xx/CX2388x подключены более чем 1 камеры) в теории равна примерно 12/(1+skip_frames). Однако на практике, в системах без использования единой внешней системы синхронизации камер (например, от переменки 24В), для устранения артефактов,
...
array(
  ...
  'type'    => 'CHECK',
  'def_val' => 0,
  'desc'    => 'Фильтр для <b>устранения эффекта &quot;расчёски&quot; (deinterlacing)</b> для кадров с размером по вертикали более 288. При LINE_DOUBLING происходит захват полукадра. При применении других фильтров происходит <b>захват полного кадра с отличным качеством</b>, но несколько повышается загрузка процессора системы. Читайте подробнее о фильтрах в документации.<br>По умолчанию: <b>LINE_DOUBLING</b>.',
  'flags'   => $F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '3.2',
  'subcats' => NULL,
  'mstatus' => 2,
...
array(
  ...
  'type'    => 'BOOL',
  'def_val' => 1,
  'desc'    => '<b>&quot;Врезать&quot; в кадр информационные строки</b> (название камеры, дата/время и др.).<p><b>Особенность по сетевым камерам</b>: если и захват и запись в MJPEG, то для записи и для передачи сетевым клиентам используются оригинальные JPEG-кадры, полученные с камер камер, т.е. параметр действует только при локальном просмотре в avreg-mon. Для ip-камер рекомендум включить наложение текста (обязательно даты/времени) на кадр на самой камере .</p>По умолчанию: <b>Вкл</b>.',
  'flags'=>$F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '5.1',
  'subcats' => '5.1.1',
  'mstatus' => 2,
...
<br /><br />По умолчанию: <b>160</b>, допустимые значения [140..255].',
  'flags'   => $F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '5.1.2',
  'subcats' => NULL,
  'mstatus' => 2,
...
<br /><br />По умолчанию: <b>80</b>, допустимые значения [0..110].',
  'flags'   => $F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '5.1.2',
  'subcats' => NULL,
  'mstatus' => 2,
...
<li><b>[10..50]</b> - <b>&quot;ручная&quot;</b> точная статическая настройка, начните с 30, если не устраивает авторегулировка.</li>
</ul>
Если автоматическая регулировка вас не устраивает, то при определении оптимального значения постарайтесь добиться 3 целей:
<ul>
<li>стабилизации diffs<sup>*</sup> на спокойных кадрах и при любой освещённости, например, c отклонением от среднего не более чем на 10%;</li>
<li>существенного изменения diffs<sup>*</sup> на неспокойных кадрах (при движении), например, минимум на 20% даже при самом медленном движении самого малого, интересующего вас, объекта в кадре.</li>
</ul>
<p>Примечание: diffs - количество пикселей, &quot;прошедших&quot; порог &#171;<span class="param">noise_threshold</span>&#187;,
отображается в правом верхнем углу кадра при просмотре в локальном вьювере &#171;avreg-mon&#187;, при &quot;включенных&quot; &#171;<span class="param">text2img</span>&#187; и &#171;<span class="param">text_changes</span>&#187;.
</p>
По умолчанию: <b>не заполнено, т.е. авторегулировка</b>.',
  'flags'=>$F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '5.1.3',
  'subcats' => NULL,
  'mstatus' => 2,
...
<p>Допустимые значения: [-5 .. +5], 0 - с нулевой (или без) коррекции, положительные значения - увеличить порог (увеличить чувстительность), отрицательные - уменьшить (&quot;загрубить&quot;).</p>
...
<p>Значение &#171;<span class="param">diff_pxls_threshold</span>&#187; должно определяться в зависимости от <b>мин. размера и мин. скорости отслеживаемых объектов</b>. Допустимо указывать как относительное изменение значения diff в процентах % (со знаком % в конце числа, например 15%), так и точное абсолюное значение порога (без знака %, например 2000). Значения 0% или 0 отключают детекцию.</p>
По умолчанию: <b>10%</b>, т.е. в пределах -10%..+10% &quot;<i>в Багдаде всё спокойно</i>&quot;.',
  'flags'=>$F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '5.1.3',
  'subcats' => NULL,
  'mstatus' => 2,
...
array(
  ...
  'type'    => 'BOOL',
  'def_val' => 1,
  'desc'    => '<b>Фильтрация</b> эффектов <b>быстрого кратковременного дрожания</b> в кадре <b>при мультиплексировании каналов</b> (когда к одному видеодекодеру  BT878/SAA71xx/CX2388x подключено сразу несколько камер). Для сетевых видеокамер не используется.<br><br>По умолчанию: <b>Вкл</b>.',
  'flags'=>$F_IN_DEF | $F_IN_CAM,
  'cats'    => '5.1.3',
  'subcats' => NULL,
  'mstatus' => 2,
...
записанные видеофайлы можно будет смотреть divx-кодеками и, возможно, на бытовых DVD-проигрывателях (примечание: по-умолчанию, для &#171;avi/mpeg4&#187; используется тег &#171;FMP4&#187;,
который понимают только медиа-проигрыватели, использующие ffmpeg(*nix)/ffdshow(win).
