...
array(
  ...
  'type'    => 'INT',
  'def_val' => 1,
  'desc'    => '<strong>Перед захватом кадра, пропускать это количество кадров</strong>.
<p>Напрямую влияет на <strong>скорость(fps) видеозахвата</strong>.</p>
<p><strong>Скорость захвата с немультиплексируемых камер</strong> (к одному видеодекодеру BT878/SAA7134/CX2388x подключена только 1 камера) будет равна <strong>25/(1+skip_frames)</strong>.
Т.е. skip_frames=0 - fps=25, skip_frames=2 - fps=12, skip_frames=3 - fps=6, и т.д.</p>
<p><strong>Скорость захвата с мультиплексируемых камер</strong> (к одному видеодекодеру BT878/SAA7134/CX2388x подключены более чем 1 камеры) в теории равна примерно 12/(1+skip_frames). Однако на практике, в системах без использования единой внешней системы синхронизации камер (например, от переменки 24В), для устранения артефактов,
возникающих из-за сбоев синхронизации видеодекодера при после переключения каналов, <strong>настоятельно не рекомендуется устанавливать значение менее чем 3. В этом случае, максимальная скорость: 5-6 fps при 2-х камерах на видеодекодер, 3-4 при 3-х и 2 fps при 4-х</strong>.</p>
По умолчанию: <b>1</b>.',
  'flags'=>$F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '3.2',
  'subcats' => NULL,
...
array(
  ...
  'type'    => 'CHECK',
  'def_val' => 0,
  'desc'    => 'Фильтр для <strong>устранения эффекта &quot;расчёски&quot; (deinterlacing)</strong> для кадров с размером по вертикали более 288. При LINE_DOUBLING происходит захват полукадра. При применении других фильтров происходит <b>захват полного кадра с отличным качеством</b>, но несколько повышается загрузка процессора системы. Читайте подробнее о фильтрах в документации.<br>По умолчанию: <b>LINE_DOUBLING</b>.',
  'flags'   => $F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '3.2',
  'subcats' => NULL,
  'mstatus' => 2,
...
array(
  ...
  'type'    => 'BOOL',
  'def_val' => 1,
  'desc'    => '<b>&quot;Врезать&quot; в кадр информационные строки</b> (название камеры, дата/время и др.).<br><br><b>Особенность по сетевым камерам</b>: если захват и запись в MJPEG, то выключено при записи. Рекомендум включить наложение текста на кадр на самой камере (обязательно даты и времени).<br><br>По умолчанию: <b>Вкл</b>.',
  'flags'=>$F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '5.1',
  'subcats' => '5.1.1',
  'mstatus' => 2,
...
array(
  ...
  'type'    => 'BOOL',
  'def_val' => 1,
  'desc'    => '<strong>Контролировать среднее значение яркости в кадре или нет</strong>.<p>Используется для получения событий засветки и затемнения камеры, автоподстройки яркости аналоговых плат видеозахвата, а также может быть использовано в алгоритмах программного детектора движения.</p>По умолчанию: <b>Вкл</b>.',
  'flags'=>$F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '5.1',
  'subcats' => '5.1.2',
  'mstatus' => 2,
...
array(
  ...
  'type'    => 'INT',
  'def_val' =>  160,
  'desc'    => '<strong>Максимальный порог</strong> среднего значения яркости в кадре,
<br />при котором <strong>вы считаете</strong>, что камера подверглась <strong>засветке</strong>.
<br /><br />По умолчанию: <strong>160</strong>, допустимые значения [140..255].',
  'flags'   => $F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '5.1.2',
  'subcats' => NULL,
  'mstatus' => 2,
...
array(
  ...
  'type'    => 'INT',
  'def_val' =>  160,
  'desc'    => '<strong>Минимальный порог</strong> среднего значения яркости в кадре,
<br />при котором <strong>вы считаете</strong>, что камера подверглась <strong>затемнению</strong>.
<br /><br />По умолчанию: <strong>80</strong>, допустимые значения [0..110].',
  'flags'   => $F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '5.1.2',
  'subcats' => NULL,
  'mstatus' => 2,
...
array(
  ...
  'type'    => 'INT',
  'def_val' =>  5,
  'desc'    => '<strong>Гистерезиз (в секундах) принятия решения о засветке/затемнении кадра</strong>.
...
По умолчанию: <strong>5 сек.</strong>, допустимые значения [1..60].',
  'flags'   => $F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '5.1.2',
  'subcats' => NULL,
  'mstatus' => 2,
