...
array(
   ...
   <p>Действует только для камер с выборочным режимом записи <nobr>({rec_mode} = &quot;Выборочно&quot;)</nobr>.
   Если на целевой (другой) камере также включен свой собственный детектор движения <nobr>({motion_detector} = Вкл.)</nobr>,
   управлять записью будут оба детектора. <span class="warntext">Камеры из списка должны обрабатываться в одном <a href="http://avreg.net/manual_applications_smp.html" targer="_blank">профиле (процессе)</a> с этой камерой</span>.</p>
   <p>Пример списка: 2-5, 11, 20</p>
   По умолчанию: &quot;пусто&quot; - управлять записью только своей камеры.',
   'flags'   => $F_RELOADED | $F_IN_CAM,
   'cats'    => '5.1.3',
   'subcats' => NULL,
   'mstatus' => 1,
