...
$recording_format   = array('avi', 'mov', 'mp4', 'mjpeg', 'webm', 'matroska', 'wav', 'mp2', 'amr', 'flac');
$rec_vcodec         = array('mpeg4', 'h264', 'mjpeg', 'h263', 'h263p', 'msmpeg4v2');
$rec_acodec         = array('mp2','flac', 'aac', 'pcm', 'adpcm', 'libgsm', 'libgsm_ms', 'libopencore_amrnb'); 
...
$PARAMS = array(
   ...
   array(
      ...
      Примечания:
      <ul>
      <li>при захвате <i>с сетевых устройств (&laquo;video_src=http/rtsp&raquo;)</i>:
      <ul>
      ...
      <li>при захвате <i>с video4linux устройств (&laquo;video_src=video4linux&raquo;)</i>:
      <ul>
...
array(
   ...
   'type'    => $INT_VAL,
   'def_val' => 30,
   'desc'    => 'Минимальный интервал в сек. &quot;спокойствия&quot; детектора движения, после чего возникает <b>событие - &quot;ОКОНЧАНИЕ СЕССИИ ДВИЖЕНИЯ&quot;</b> и система переходит в состояние ожидания &quot;НАЧАЛА НОВОЙ СЕССИИ ДВИЖЕНИЯ&quot; в зоне наблюдения этой камеры. Примечание: по этому событию безусловно закрывается открытый  медиа-файл, независимо от пределов file_max_megabytes и file_max_minutes.<br><br>Допустимые значения от <b>10 до 600 сек</b>. По умолчанию: <b>30 сек</b>.',
   'flags'=>$F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
   'cats'    => '5.1.3',
   'subcats' => NULL,
   'mstatus' => 1,
...
array(
   ...
   'type'    => $CHECK_VAL,
   'def_val' => 1,
   'desc'    => '<b>Формат файла для записи</b>.<p>Если формат не задан, то программа попытается подобрать подходящий формат с учётом кодеков входных(захват) медиаданных. <b>Установка несовместимого с кодеками входных(захват) медиаданных формата при невозможности перекодирования приведёт к невозможности записи</b>.</p><p>По умолчанию: не установлено - &#171;авто&#187;.</p>',
   'flags'   => $F_RELOADED | $F_IN_DEF | $F_IN_CAM,
   'cats'    => '11',
   'subcats' => NULL,
   'mstatus' => 1,
...
array(
   ...
   'type'    => $INT_VAL,
   'def_val' => 5,
   'desc'    => '<b>Максимальный размер видеофильма</b> в МегаБайтах.<br><br>По достижению любого, этого или file_max_minutes (см. ниже) пределов, запись продолжится в новый файл. При включенном детекторе движения, событие &#171;окончание сессии движения&#187; закроет файл независимо от любых установленных пределов на размер и продолжительность.
   <br><br>Если Вы <b>хотите иметь огромные видеофильмы, например, в AVI-файлах</b> (по аналогии с DVD, VIDEOCD и т.п.), <b>подумайте</b> - удобно ли будет с ними работать в режиме доступа к видеоархиву по сети?<br><br>Допустимые значения: от 1 до 1000(1Гб). &nbsp;По умолчанию: <b>5 Mb</b>.',
   'flags'   => $F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
   'cats'    => '11',
   'subcats' => NULL,
   'mstatus' => 1,
...
array(
   ...
   'type'    => $INT_VAL,
   'def_val' => 60,
   'desc'    => '<b>Максимальная продолжительность</b> видеофильма в <b>минутах</b>.<br><br>По достижению любого, этого или file_max_megabytes (см. выше) пределов, запись продолжится уже в новый файл.
...
array(
   ...
   'type'    => $INT_VAL,
   'def_val' => 5,
   'desc'    => '<b>Степень сжатия кодируемого медиа</b> или некий коэффициент, <b>обратный  качеству изображения</b>. Параметр соответствует ffmpeg-параметру qscale для режима VBR (переменный битрейт с целью удержания постоянного качества изображения/звука.<br><br>Прим.: <b>не используется при записи в оригинальном формате захвата</b>.<br><br>Допустимые значения: <b>2</b>(лучшее качество и макс. размер файла) - <b>30</b>(хуже, но меньше).<br/>По умолчанию <b>5</b>(оптимально).',
   'flags'   => $F_RELOADED | $F_IN_DEF | $F_IN_CAM,
   'cats'    => '11',
   'subcats' => NULL,
   'mstatus' => 1,
...
array(
   ...
   'type'    => $CHECK_VAL,
   'def_val' => '',
   'desc'    => 'Видеокодек для записи видео на жесткий диск.
...
array(
   ...
   'type'    => $CHECK_VAL,
   'def_val' => '',
   'desc'    => 'Видеокодек для записи аудио на жесткий диск.
   По умолчанию: <b>не установлено - &#171;авто&#187;</b>',
   'flags'   => $F_RELOADED | $F_IN_DEF | $F_IN_CAM,
   'cats'    => '11.2',
   'subcats' => NULL,
