...
$deinterlacers = array('LINE_DOUBLING','LINEAR_BLEND');
...
array(
  ...
  'type'    => 'CHECK',
  'def_val' => 'netcam',
  'desc'    => '<b>Тип  видеокамеры</b>:<ul><li><b>netcam</b> - сетевые IP-камеры или видеосервера;</li><li><b>v4l</b> - (video for linux) аналоговые видеокамеры, подключаемые к PCI-платам видеозахвата или ТВ-тюнерам.</li></ul>По умолчанию: &quot;<b>netcam</b>&quot;.',
  'flags'=>$F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '3',
  'subcats' => '3.1;3.2',
  'mstatus' => 2,
...
<ul><li><b>Для камер, подключенных к платам видеозахвата,</b> установите пропорционально разрешающей способности видеокамеры с учётом требуемого качества и ресурсов сервера. Для ВСЕХ каналов ОДНОГО устройства видеозахвата (более точно - для одной микросхемы видеокодера bt878a,saa7130,...) должно быть ОДНО значение:<br><b>384x288, 480x360, 560x420, 640x480, 720x540, 720x576 (макс.&nbsp;saa713x), 768x576 (макс.&nbsp;bt878a)</b>.</li><li><b>Для сетевых</b> камер выберите из списка размеры изображения <b>которое реально выдает ip-камера</b> или видеосервер. Если вы не нашли в нашем списке нужного разрешения, сообщите нам.</li></ul>По умолчанию: <b>640x480</b>.',
  'flags'=>$F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '3',
  'subcats' => '3.1;3.2',
  'mstatus' => 2,
...
array(
  ...
  'type'    => 'BOOL',
  'def_val' => 0,
  'desc'    => 'Какой кадр <b>ожидает</b> получить программа: <b>цветной или монохромный</b>. Важный параметр, <b>ставьте реальные значения</b>.<br><br>Для аналоговых плат видеозахвата смотрите также описание параметра &#171;<b>norm</b>&#187;.<br><br>По умолчанию: <b>Выкл. (ч/б в/к)</b>.',
  'flags'=>$F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '3',
  'subcats' => '3.1;3.2',
  'mstatus' => 2,
...
array(
  ...
  'type'    => 'INT',
  'def_val' => 1,
  'desc'    => '<b>Пропускать это количество кадров перед захватом каждого кадра</b>.
Применение этого параметра действует по-разному в зависимости от схемы подключения камер к видеокодерам.
<ul>
<li><b>Немультиплексируемые камеры</b>, т.е. когда к одному видеокодеру BT878/SAA71xx/CX2388x подключена только 1 камера. В этом случае значение &#171;<span class="param">skip_frames</span>&#187; напрямую влияет на <b>скорость (fps) видеозахвата</b>, которая будет равна <b>fps&nbsp;=&nbsp;25/(1+skip_frames)</b>.</li>.
<li><b>Мультиплексируемые каналы (камеры)</b>, т.е. когда к одному видеокодеру подключена более чем 1 камера. В системах без использования единой внешней системы синхронизации камер (например, от переменки 24В), <b>для устранения артефактов,
возникающих из-за сбоев синхронизации видеокодера при каждом переключении каналов</b>, рекомендуется устанавливать значение не&nbsp;менее чем 2. При хороших условиях (одинаковые  качественные  камеры и их эл. соединения) максимальная скорость получается такая: 5-6 fps при 2-х камерах на видеокодер, 3-4 при 3-х и 2 fps при 4-х</b>.</p>
</li></ul>
По умолчанию: <b>1</b>.',
...
array(
  ...
  'type'    => 'CHECK',
  'def_val' => 0,
  'desc'    => 'Фильтр для <b>устранения эффекта &quot;расчёски&quot/&quotгребёнки&quot;</b> для кадров с размером по вертикали более 288 пикселей.<p>При использовании LINE_DOUBLING, захват осуществляется полукадрами с последующим чрезстрочным дублированием линий полукадра для получения полного кадра. При использовании LINEAR_BLEND полный кадр восстанавливается из 2-х полукадров, т.е. видеоинформация не теряется, однако несколько повышается нагрузка на CPU.</p>По умолчанию: <b>LINE_DOUBLING</b>.',
  'flags'   => $F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '3.2',
  'subcats' => NULL,
  'mstatus' => 2,
...
array(
  ...
  'type'    => 'INT',
  'def_val' => 3,
  'desc'    => '<b>Предзапись</b>. Если включен режим записи на диск, будет также записано это количество &quot;спокойных&quot; кадров, захваченных перед КАЖДЫМ кадром, на котором сработал детектор движения. <b>Существенно увеличивает требование к объёму оперативной памяти</b>.<br><br>Допустимые значения от <b>0 до 20</b>. По умолчанию: <b>3 кадра</b>.',
  'flags'=>$F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '5.1.3',
  'subcats' => NULL,
  'mstatus' => 2,
...
array(
  ...
  'type'    => 'INT',
  'def_val' => 3,
  'desc'    => '<b>Послезапись</b>. Если включен режим записи на диск, будет также записано это количество &quot;спокойных&quot; кадров, захваченных после КАЖДОГО кадра, на котором сработал детектор движения.<br><br>Допустимые значения от <b>0 до 20</b>. По умолчанию: <b>3 кадра</b>.',
  'flags'=>$F_RELOADED | $F_BASEPAR | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '5.1.3',
  'subcats' => NULL,
  'mstatus' => 2,
...
array(
 ...
 записывать кадры <b>JPEG через каждые snapshot_interval секунды</b>.
<div style="color: Red;">
Использование этой возможности почти всегда приводит к замедлению работы файловой системы и, как следствие, системы в целом. Эта возможность будет исключена в следующих версиях.</div>
По умолчанию: <b>0 - Выкл</b>.',
  'flags'=>$F_RELOADED | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '11.1',
  'subcats' => NULL,
  'mstatus' => 2,
...
array(
  ...
  '<li>скорость кодирования заметно больше при кодировании кодеком MP2 (соответственно нагрузка на CPU меньше),</li>'.
