...
$strOnlySelDet='Доступно только при выборочном режиме записи (<em>save_video</em>=&laquo;Выборочный&raquo;) и включенном детекторе движения (<em>motion_detector</em>=&laquo;Вкл.&raquo;).';
...
<li>&#171;Выборочный&#187; (по-умолчанию) - режим, при котором запись инициируется и прекращается сработками детектора движения (начало и окончания сессий движения, детектор движения должен быть включен и настроен) и/или внешними командами (пока не реализовано).</li>
</ul>
...
array(
  ...
  'type'    => $INT_VAL,
  'def_val' => 0,
  'desc'    => '<em>Длительность в секундах 1-го спапшотного периода</em> с момента начала выборочной записи (например, с начала каждого сеанса движения; см. описание параметра <em>save_video</em>), в котором (периоде) на диск со скоростью не более чем 1 fps (<em>1 кадр в секунду</em>) сохраняются кадры-отметки в формате jpeg, <em>дополнительно к записи в основной формат</em> (см. описание <em>V.save_fmt</em> выше). На диск записываются любые кадры,  как &quot;c движением&quot; так и &quot;спокойные&quot;, т.е. без учёта детектора движения.
<p>'.$strOnlySelDet.'</p>Допустимые значения: [0..30] секунд (макс. 60 кадров).
...
<p>'.$strOnlySelDet.'</p>Допустимые значения: [0..600] секунд (макс. 120 кадров).
<br>По умолчанию: <b>0 - не записывать отметки в этом интервале</b>.',
  'flags'   => $F_RELOADED | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '11.1',
  'subcats' => NULL,
...
<li>%W - последние две цифры текущей сессии движения (при вкл. детекторе);</li>
...
array(
  ...
  '<li>сброс битов чтения/поиска для &quot;остальных&quot; пользователей (0750) заблокирует работу с архивом через веб-интерфейс и по FTP в конфигурации веб-сервера по-умолчанию;</li>'.
  '<li>параметром &quot;store_path_fmt&quot; нужно полностью разделить все части каталогов с отличными &quot;dirs_mode&quot;.</li>'.
  '</ul>'.
  'По умолчанию: <b>0775</b> - поиск/чтение для всех, запись/удаление только пользователю {user} или членам группы {group} (см. настройки avregd{} в файле avreg.conf).',
  'flags'=>$F_RELOADED | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '11',
  'subcats' => NULL,
  'mstatus' => 2,
...
array(
  ...
  '</ul>'.
  'По умолчанию: <b>не задано</b>, то есть {group} в настройках avregd{} файла avreg.conf.',
  'flags'=>$F_RELOADED | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '11',
  'subcats' => NULL,
  'mstatus' => 2,
...
array(
  ...
  'max_len' => 4,
  'def_val' => '0755',
  'desc'    => '<b>Права доступа на записываемые медиа-файлы</b>, замечания аналогично &quot;dirs_mode&quot;, см. выше.'.
  '<br />По умолчанию: <b>0664</b> - чтение для всех, запись/удаление только пользователю {user} или членам группы {group} (см. настройки avregd{} в файле avreg.conf).',
  'flags'=>$F_RELOADED | $F_IN_DEF | $F_IN_CAM,
  'cats'    => '11',
  'subcats' => NULL,
  'mstatus' => 2,
