...
array(
  ...
  'type'    => $INT_VAL,
  'def_val' => 7,
  'desc'    => '<b>Желаемая скорость видеозахвата в кадрах в секунду</b>.
<div>Если значение параметра не установлено явно или равно 0, то программа автоматически будет использовать сдедующие, разумные для большинства случаев, значения:
</div>
<ul>
<li>при захвате с сетевых ip-камер и видеосерверов:
<ul>
<li>потоковые данные (mjpeg, ) - без ограничений (правильно ограничивать скорость отдаваемого потока на самой камере)</li>
<li>одиночными кадрами jpeg (снапшотами) - 5 fps</li>
</ul>
<li>
<li>при захвате с устройств video4linux (платы видеозахвата, USB-камеры):
<ul>
<li>с немультиплексируемых устройств (1 камера на 1 видеокодере) - 15 fps</li>
<li>c мультиплексируемых устройств (несколько камер на 1 видеокодере) - 12/кол-во каналов на устройстве</li>
</ul>
</li>
</ul>
По умолчанию: не определено, т.е. автоматически.',
...
array(
  ...
  'type'    => $BOOL_VAL,
  'def_val' => 0,
  'desc'    => 'Программно <b>масштабировать</b> выводимое видео (<b>просмотр</b> в реальном времени и в архиве, через веб-интерфейс) увеличивая разрешение по вертикали в 2 раза. Установите, если для снижения нагрузки на видеосервер вы используете  <b>захват полукадрами</b>, т.е. обычно тогда, когда разрешение по вертикали <nobr>&gt;= 288(pal)/240(secam)</nobr>, например: 720х288 или 640х240.<br><br>По умолчанию: <b>Выкл. - не масштабировать.</b>.',
...
array(
  ...
  'type'    => $BOOL_VAL,
  'def_val' => 0,
  'desc'    => 'Принудительное использование устаревшего API video4linux1 (версия 1). Может оказаться полезным при захвате с устройств (часто USB-камеры) с некачественными (сырыми) драйверами video4linux2 (версия  2).<br>По умолчанию: <b>не установлено</b> - поддерживаемую версию API сообщает драйвер.',
  'flags'   => $F_IN_DEF | $F_IN_CAM,
  'cats'    => '3.2',
  'subcats' => NULL,
  'mstatus' => 2,
