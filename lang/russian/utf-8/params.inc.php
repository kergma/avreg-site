...
$deinterlacers = array('LINE_DUP','LINEAR_BLEND');
...
Применение этого параметра действует по-разному в зависимости от схемы подключения камер к видеодекодерам.
<ul>
<li><b>Немультиплексируемые камеры</b>, т.е. когда к одному видеодекодеру BT878/SAA71xx/CX2388x подключена только 1 камера. В этом случае значение &#171;<span class="param">skip_frames</span>&#187; напрямую влияет на <b>скорость (fps) видеозахвата</b>, которая будет равна <b>fps&nbsp;=&nbsp;25/(1+skip_frames)</b>.</li>.
<li><b>Мультиплексируемые каналы (камеры)</b>, т.е. когда к одному видеодекодеру подключена более чем 1 камера. В системах без использования единой внешней системы синхронизации камер (например, от переменки 24В), <b>для устранения артефактов,
возникающих из-за сбоев синхронизации видеодекодера после переключения каналов</b>, рекомендуется устанавливать значение не&nbsp;менее чем 2. При хороших условиях (одинаковые  качественные  камеры и их эл. соединения) максимальная скорость получается такая: 5-6 fps при 2-х камерах на видеодекодер, 3-4 при 3-х и 2 fps при 4-х</b>.</p>
</li></ul>
По умолчанию: <b>1</b>.',
...
<li><b>[10..50]</b> - <b>&quot;ручная&quot;</b> точная статическая настройка (совет: начните с 30 если не устраивает авторегулировка).</li>
</ul>
