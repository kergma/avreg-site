...
class PdaPaginator implements Iterator
{
    ...
    public function __construct($ary, $offset, $detail_uri_base, $conf, $limit = 10, $offset_name = 'off')
    {
    ...
    public function printAbove()
    {
        ...
        $uri = sprintf(
            ...
            $this->uri_base,
            $this->offset_name,
            ($this->elems_per_page * ($this->cur_page - 2)),
            $scale,
            $FS_size
        );
        ...
        printf(
            "<img src='%s/img/arrow_up_48x24.png' width=48 height=24 alt='пред.'></a></div>\n",
            $this->conf['prefix']
        );
    }

    public function printBelow()
    {
        $scale = isset($_GET['scl']) ? $_GET['scl'] : 0;
        $FS_size = '';
        if (isset($_GET['aw']) && isset($_GET['ah'])) {
        ...
        if ($this->cur_page < $this->all_pages_nb) {
            ...
            $uri = sprintf(
                ...
                $this->uri_base,
                $this->offset_name,
                ($this->elems_per_page * $this->cur_page),
                $scale,
                $FS_size
            );
            ...
            printf(
                "<img src='%s/img/arrow_down_48x24.png' width=48 height=24 alt='след.'></a></div>\n",
                $this->conf['prefix']
            );
        }
