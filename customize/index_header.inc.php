...
echo '<h2 class="header" align="center">' . sprintf($fmtVidServ, $named) .
    '&nbsp;&nbsp;(<a class="main_links" style="font-size: 85%;" href="' . sprintf(
        $conf['prefix'] . '/admin/key.php?sip=%s&amp;named=%s',
        $sip,
        $named
    ) . '">' . $license . '</a>)</h2>' . "\n";
print "<div  class='header' align='center'>$strYou: <a class='main_links' href='" . $conf['protocol']
    . $conf['url_demon'] . "index.php?logout=1' title='$strLogout'>$login_user@$remote_addr</a></div>\n";
    ...
    if ((width < 1024) || (height < 768)) {
        ...
        if (warn_cnt < 3) {
            alert("\nВеб-интерфейс AVReg расчитан на разрешение экрана не менее 1024x768,\nтак как размер даже " +
                "одного записанного или передаваемого видео обычно составляет\n640х480 или 756х568 пикселей. " +
                "А помимо кадров, на страницах нужно разместить и элементы управления\n\nУ Вас разрешение экрана " +
                width + " x " + height + " и работать будет неудобно.\n\nЕсли у Вас современный монитор и " +
                "Вы работаете в " + width + "x" + height + " из-за мелкого шрифта, просто переключитесь в " +
                "1280х1024 и " +
                "увеличьте системный шрифт.\n\nНормальные операционные системы делают эту операцию очень хорошо и " +
                "дополнительно умеют сглаживать шрифты.\n\n\nЭто предупреждение будет показано ещё " + (2 - warn_cnt) +
                " раз(а).");
