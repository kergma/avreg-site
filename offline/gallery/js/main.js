        ...
        init: function (holder, ajax_params) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    } else if (data.status == 'error' && data.code == '2') { //если вовремя очистки не были удалены все дубли
                        $('#matrix_load').hide();
    ...
    detail: function () {
        ...
        $(".content_item").each(function () {
            if (!$(this).hasClass("active")) {
                $(this).hide();
            } else { //сохраняем необходимые параметры активного элемента
...
//			$('#matrix_load').show();
    ...
    update: function (sp) {
                ...
                $(cells).each(function (i) {
                    ...
                    if (typeof( matrix.events[i]) != 'undefined') {
                        ...
                        if (value[7] == 'image') {
                            ...
                            } else { //если была не картинка - переустанавливаем плеер
                                matrix.setimagesize(i);
        ...
        //Включаем тултип
        $(".elem .info_block").tooltip();
        if (matrix.num > sp + matrix.cell_count) {
            matrix.num = sp;
        }
        $('.active').removeClass('active');
        $('#cell_' + matrix.num).addClass('active');
