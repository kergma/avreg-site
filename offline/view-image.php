...
?>
    ...
    <script type="text/javascript" language="JavaScript1.2">
        ...
        if ($MSIE || false !== strpos($ua, 'windows')) {
            $platform = 'win';
        } else {
            // @codingStandardsIgnoreStart
            if ( false !== strpos($ua, 'linux') || false !== strpos($ua, 'solaris') ||
                    false !== strpos($ua, 'bsd') || false !== strpos($ua, 'mac os x') ) {
                $platform = 'nix';
            } else {
              $platform = 'other';
            }
        ...
        foreach ($_ipacl_list as &$_ipacl_str) {
            ...
            // @codingStandardsIgnoreEnd
                ...
                printf(
                    "alert(\"invalid IP ACL [%s] for murl-pre-$platform param (see avreg.conf)\");\n",
        ...
        if (!$_found) { /* не нашли совпадения по ACL */
            print 'var MediaUrlPref = ' .
             'location.protocol + \'\/\/\' + location.host + WwwPrefix + MediaAlias + \'\/\';'."\n";
        }
        ?>
        // -->
    </script>
    ...
    if (!isset ($src) || empty($src)) {
        print "<div style=\"padding: 50px 30px 5px 30px; font-size: 110%;\">$strViewFrame1</div>
        <div style=\"padding: 5px 30px 30px 30px;  font-size: 110%;\">$strViewFrame2</div>\n";
    }
    print  '</div>' . "\n";
