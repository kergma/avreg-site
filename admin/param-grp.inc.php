...
{
    if (preg_match('/^\d+$/', $_cat_id)) {
        return 0;
    } elseif (preg_match('/^\d+\.\d+$/', $_cat_id)) {
        return 1;
    } elseif (preg_match(
        '/^\d+\.\d+\.\d+$/',
        $_cat_id
        ...
        return 2;
    } elseif (preg_match('/^\d+\.\d+\.\d+\.\d+$/', $_cat_id)) {
        return 3;
    }
    return -1;
}
...
if (isset($categories)) {
    if (!preg_match("/^(\d+|\d+\.?)+$/", $categories)) {
        die ('Wrong query. Your are hacker`s?');
    }
    $cats_level_in_POST = get_cats_level($categories);
    if ($cats_level_in_POST === -1) {
        die ('Wrong query. Your are hacker`s?');
    }
    push_cat($categories);
...
for ($i = 0; $i < $ccc; $i++) {
    if ($user_status > $PAR_GROUPS[$i]['mstatus']) {
        continue;
    }
    if (!preg_match('/' . $categoriesSQL . '/', $PAR_GROUPS[$i]['id'])) {
        continue;
    }
...
while ($cat_nr < $r_cnt) {
    ...
    if (isset($categories) && !strcmp($cats[$cat_nr]['id'], $categories)) {
        ...
        if (!open_cat_tree($cats[$cat_nr]['id'], $_level)) {
            ...
            print '<a href="' . $_SERVER["PHP_SELF"] . '?categories=' . $cats[$cat_nr]['id'] . $cam_req . '" title="'
                . $cats[$cat_nr]['desc'] . '">' . $cats[$cat_nr]['name'] . '</a>';
        } else {
            print '<img src="' . $conf['prefix'] . '/img/folder.open.gif" border="0">&nbsp;';
            print '<a href="' . $_SERVER["PHP_SELF"] . '?categories=' . $cats[$cat_nr]['id'] . $cam_req
                . '" style="color:' . $error_color . ';" title="' . $cats[$cat_nr]['desc'] . '">'
                . $cats[$cat_nr]['name'] . '</a>';
        }
    }
    if (!empty($cats[$cat_nr]['help_page'])) {
        print '&nbsp;&nbsp;<sup>(&nbsp;<a href="' . $cats[$cat_nr]['help_page']
            . '" target="_blank">help</a>&nbsp;)</sup>';
    }
    print '</div>' . "\n";
